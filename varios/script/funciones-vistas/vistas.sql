...
(CASE WHEN A.FECING<TO_DATE('19/06/1997','DD/MM/YYYY') THEN
 ...
 ELSE
 TO_DATE(TO_CHAR(A.FECING,'DD/')||TO_CHAR(A.FECFIN,'MM/YYYY'),'DD/MM/YYYY') END) AS FECINIREAL,
(CASE WHEN A.FECING<TO_DATE('19/06/1997','DD/MM/YYYY') THEN
 ADD_MONTHS(TO_DATE(TO_CHAR(A.FECING,'19/')||TO_CHAR(A.FECFIN,'MM/YYYY'),'DD/MM/YYYY'),1)-1
 ELSE
 ADD_MONTHS(TO_DATE(TO_CHAR(A.FECING,'DD/')||TO_CHAR(A.FECFIN,'MM/YYYY'),'DD/MM/YYYY'),1)-1 END) AS FECFINREAL,
A.MONTO,A.SALNOR,A.BONOUTI,A.BONOVAC,A.BONOADI,
ROUND(A.MONTO/30,2) +
(COALESCE((CASE WHEN C.ALICUOCON=0 THEN 0
 ELSE (SELECT COALESCE(DIAUTI,0)
  ...
  END),0)/30/12)* (CASE WHEN A.FACTORUTI= 1 THEN ROUND(A.MONTO/30,2)
                   ELSE ROUND(A.MONTO*A.FACTORUTI/365,2) END) +
 (COALESCE((CASE WHEN C.ALICUOCON=0 THEN 0
 ELSE (SELECT COALESCE(DIAVAC,0)
  ...
  END),0)/30/12)* (CASE WHEN A.FACTORVAC= 1 THEN ROUND(A.MONTO/30,2)
                   ELSE ROUND(A.MONTO*A.FACTORVAC/365,2) END) +
 (COALESCE((CASE WHEN C.ALICUOCON=0 THEN 0
 ELSE (SELECT COALESCE(diapro,0)
        FROM NPBONOCONT
        WHERE ANOVIG<=A.FECINI
        AND ANOVIGHAS>=A.FECFIN
        AND CODTIPCON=A.CODTIPCON
...
(COALESCE((SELECT COALESCE(DIAUTI,0)
        ...
        AND CODTIPCON=A.CODTIPCON
        AND DESDE<=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
        AND HASTA>=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)),0)/12)*(CASE WHEN A.FACTORUTI= 1 THEN ROUND(A.SALNOR/30,2)
                                                                                                           ELSE ROUND(A.SALNOR*A.FACTORUTI/365,2) END) +
 (COALESCE((SELECT COALESCE(DIAVAC,0)
        ...
        AND CODTIPCON=A.CODTIPCON
        AND DESDE<=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
        AND HASTA>=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)),0)/12)*(CASE WHEN A.FACTORVAC= 1 THEN ROUND(A.SALNOR/30,2)
                                                                                                              ELSE ROUND(A.SALNOR*A.FACTORVAC/365,2) END) +
 (COALESCE((SELECT COALESCE(diapro,0)
        FROM NPBONOCONT
        WHERE ANOVIG<=A.FECINI
        AND ANOVIGHAS>=A.FECFIN
        AND CODTIPCON=A.CODTIPCON
...
(COALESCE((SELECT COALESCE(DIAUTI,0)
        ...
        AND DESDE<=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
        AND HASTA>=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)),0)/12)*(CASE WHEN A.FACTORUTI= 1 THEN ROUND(A.SALNOR/30,2)
                                                                                                           ELSE ROUND(A.SALNOR*A.FACTORUTI/365,2) END) as ALIUTI,
 (COALESCE((SELECT COALESCE(DIAVAC,0)
        FROM NPBONOCONT
        WHERE ANOVIG<=A.FECINI
        AND ANOVIGHAS>=A.FECFIN
        ...
        AND DESDE<=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
        AND HASTA>=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)),0)/12)*(CASE WHEN A.FACTORVAC= 1 THEN ROUND(A.SALNOR/30,2)
                                                                                                              ELSE ROUND(A.SALNOR*A.FACTORVAC/365,2) END) as ALIVAC,
 (COALESCE((SELECT COALESCE(diapro,0)
        FROM NPBONOCONT
        WHERE ANOVIG<=A.FECINI
        AND ANOVIGHAS>=A.FECFIN
        AND CODTIPCON=A.CODTIPCON
        AND DESDE<=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
        AND HASTA>=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)),0)/12)*ROUND(A.SALNOR/30,2) as ALIADI,
A.CODTIPCON,
...
COALESCE((CASE WHEN (SELECT Z.FACTORBONFINANOFRA
           FROM NPASIEMPCONT Y,NPDEFESPPARPRE Z
           WHERE Y.CODEMP=A.CODEMP
           AND Y.CODNOM=Z.CODNOM) IS NOT NULL THEN
           (SELECT Z.FACTORBONFINANOFRA
           FROM NPASIEMPCONT Y,NPDEFESPPARPRE Z
           WHERE Y.CODEMP=A.CODEMP
           AND Y.CODNOM=Z.CODNOM)
           ELSE 1
            END),0) AS FACTORUTI,
COALESCE((CASE WHEN (SELECT Z.FACTORBONVACFRA
           FROM NPASIEMPCONT Y,NPDEFESPPARPRE Z
           WHERE Y.CODEMP=A.CODEMP
           AND Y.CODNOM=Z.CODNOM) IS NOT NULL THEN
           (SELECT Z.FACTORBONVACFRA
           FROM NPASIEMPCONT Y,NPDEFESPPARPRE Z
           WHERE Y.CODEMP=A.CODEMP
           AND Y.CODNOM=Z.CODNOM)
           ELSE 1
...
COALESCE((SELECT SUM(X.MONTO)
           FROM NPHISCON X,NPVACDEFGEN Y
           WHERE X.CODEMP=A.CODEMP
           AND X.FECNOM>=B.FECINI
           AND X.FECNOM<=B.FECFIN
           AND X.CODCON=Y.CODCONUTI),0) AS BONOUTI,
COALESCE((SELECT SUM(X.MONTO)
           FROM NPHISCON X,NPVACDEFGEN Y
           WHERE X.CODEMP=A.CODEMP
           AND X.FECNOM>=B.FECINI
           AND X.FECNOM<=B.FECFIN
           AND X.CODCON=Y.CODCONVAC),0) AS BONOVAC,
COALESCE((SELECT SUM(X.MONTO)
           FROM NPHISCON X,NPVACDEFGEN Y
           WHERE X.CODEMP=A.CODEMP
           AND X.FECNOM>=B.FECINI
           AND X.FECNOM<=B.FECFIN
           AND X.CODCON=Y.CODCONADI),0) AS BONOADI,
