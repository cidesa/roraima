...
(COALESCE((CASE WHEN C.ALICUOCON=0 THEN 0
 ELSE (SELECT COALESCE(DIAUTI,0)
  ...
  END),0)/30/12)*
 COALESCE((CASE WHEN C.ALICUOCON=0 THEN ROUND(A.MONTO/30,2)
           ELSE (CASE WHEN (SELECT (CASE WHEN CALINC='S'
                                    THEN 12+(COALESCE(DIAUTI,0)/30)+(COALESCE(DIAVAC,0)/30)+(COALESCE(DIAPRO,0)/30)
                                    ELSE 1 END)
                            FROM NPBONOCONT
                            WHERE ANOVIG<=A.FECINI
                            AND ANOVIGHAS>=A.FECFIN
                            AND CODTIPCON=A.CODTIPCON
                            AND DESDE<=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
                            AND HASTA>=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END))=1
                 THEN ROUND(A.MONTO/30,2)
                 ELSE ROUND((SELECT (CASE WHEN CALINC='S'
                                     THEN 12+(COALESCE(DIAUTI,0)/30)+(COALESCE(DIAVAC,0)/30)+(COALESCE(DIAPRO,0)/30)
                                     ELSE 1 END)
                             FROM NPBONOCONT
                             WHERE ANOVIG<=A.FECINI
                             ...
                             AND CODTIPCON=A.CODTIPCON
                             AND DESDE<=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
                             AND HASTA>=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END))*A.MONTO/365,2)
                 END)
           END),0) +
 (COALESCE((CASE WHEN C.ALICUOCON=0 THEN 0
...
(COALESCE((SELECT COALESCE(DIAUTI,0)
        ...
        AND CODTIPCON=A.CODTIPCON
        AND DESDE<=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
        AND HASTA>=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)),0)/12)*
 COALESCE((CASE WHEN C.ALICUOCON=0 THEN ROUND(A.MONTO/30,2)
           ELSE (CASE WHEN (SELECT (CASE WHEN CALINC='S'
                                    THEN 12+(COALESCE(DIAUTI,0)/30)+(COALESCE(DIAVAC,0)/30)+(COALESCE(DIAPRO,0)/30)
                                    ELSE 1 END)
 ...
 (COALESCE((SELECT COALESCE(DIAVAC,0)
        ...
        AND CODTIPCON=A.CODTIPCON
        AND DESDE<=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
        AND HASTA>=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)),0)/12)*
 COALESCE((CASE WHEN C.ALICUOCON=0 THEN ROUND(A.MONTO/30,2)
           ELSE (CASE WHEN (SELECT (CASE WHEN CALINC='S'
                                    THEN 12+(COALESCE(DIAUTI,0)/30)+(COALESCE(DIAVAC,0)/30)+(COALESCE(DIAPRO,0)/30)
                                    ELSE 1 END)
...
COALESCE((CASE WHEN (SELECT (CASE WHEN Z.FACTORBONFINANOFRA <> 0 THEN Z.FACTORBONFINANOFRA
                              ELSE 1 END)
           FROM NPASIEMPCONT Y,NPDEFESPPARPRE Z
           WHERE Y.CODEMP=A.CODEMP
           AND Y.CODNOM=Z.CODNOM) IS NOT NULL THEN
           (SELECT (CASE WHEN Z.FACTORBONFINANOFRA <> 0 THEN Z.FACTORBONFINANOFRA
                    ELSE 1 END)
           FROM NPASIEMPCONT Y,NPDEFESPPARPRE Z
           WHERE Y.CODEMP=A.CODEMP
           ...
           ELSE 1
            END),0) AS FACTORUTI,
COALESCE((CASE WHEN (SELECT (CASE WHEN Z.FACTORBONVACFRA <> 0 THEN Z.FACTORBONVACFRA
                             ELSE 1 END)
           FROM NPASIEMPCONT Y,NPDEFESPPARPRE Z
           WHERE Y.CODEMP=A.CODEMP
           AND Y.CODNOM=Z.CODNOM) IS NOT NULL THEN
           (SELECT (CASE WHEN Z.FACTORBONVACFRA <> 0 THEN Z.FACTORBONVACFRA
                    ELSE 1 END)
           FROM NPASIEMPCONT Y,NPDEFESPPARPRE Z
           WHERE Y.CODEMP=A.CODEMP
           AND Y.CODNOM=Z.CODNOM)
