...
(COALESCE((SELECT COALESCE(DIAUTI,0)
 ...
 (COALESCE((SELECT COALESCE(DIAVAC,0)
        ...
        AND CODTIPCON=A.CODTIPCON
        AND DESDE<=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
        AND HASTA>=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)),0)/12)*ROUND(A.SALNOR/30,2) +
...
(COALESCE((SELECT COALESCE(DIAUTI,0)
        ...
        AND CODTIPCON=A.CODTIPCON
        AND DESDE<=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
        AND HASTA>=(CASE WHEN COALESCE(B.ANTAP,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)),0)/12)*ROUND(A.SALNOR/30,2) as ALIUTI,
 (COALESCE((SELECT COALESCE(DIAVAC,0)
        FROM NPBONOCONT
        WHERE ANOVIG<=A.FECINI
        AND ANOVIGHAS>=A.FECFIN
        AND CODTIPCON=A.CODTIPCON
        AND DESDE<=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)
        AND HASTA>=(CASE WHEN COALESCE(B.ANTAPVAC,'N')='N' THEN A.ANNOANTIG ELSE A.ANNOANTIGPUB END)),0)/12)*ROUND(A.SALNOR/30,2) as ALIVAC,
 (COALESCE((SELECT COALESCE(diapro,0)
        FROM NPBONOCONT
        WHERE ANOVIG<=A.FECINI
        AND ANOVIGHAS>=A.FECFIN
