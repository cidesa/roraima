...
if ($driver == 'postgres') {
	...
	$db->PConnect('localhost','SIMA011','SIMA011','5432','SIMA');
}
...
if ($driver == 'oci8') {
	...
	$db->Connect('SOPORTE01','SIMA013','SIMA013','SIMA');
	
//$sql = "select * from (select  ID, firstname as \"First Name\", lastname as \"Last Name\" from adoxyz  	 order  by  (1))";
$sql = "select * from (select  CODCTA as \"CUENTA\", DESCTA as \"DESCRIPCION\" from CONTABB 
	 order  by  (1))";
