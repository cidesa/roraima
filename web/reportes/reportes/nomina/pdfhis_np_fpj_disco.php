	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->sql="SELECT
						...
						E.CODEMP <= ('".$this->codemphas."') AND 	 ".$especial."
...
H.codcar=F.codcar AND
E.CODEMP=H.CODEMP AND
E.CODEMP >=  RPAD('".$this->codempdes."',16,' ') AND
E.CODEMP <= RPAD('".$this->codemphas."',16,' ') AND
H.CODCON = B.CODCON AND
upper(rtrim(B.TIPO))='R'  AND
H.FECNOM>=to_date('".$this->fechades."','dd/mm/yyyy') AND
H.FECNOM<=to_date('".$this->fechahas."','dd/mm/yyyy') AND
B.CODNOM=H.CODNOM AND
A.CODTIPAPO=B.CODTIPAPO AND
...
//print $this->sql;
		...
		function Header()
		{
			$this->cab->poner_cabecera($this,"Fondo de Pensiones y Jubilados ","l","s");
		...
		{   $this->setFont("Arial","B",9);
			...
			while (!$tb->EOF){
			   ...
			   if ($cod_nom!=$tb->fields["codnomapo"]){
					...
					$this->cell(30,10,"Retención 0%");
					$this->cell(30,10,"Aporte 0%");
					...
					$this->setFont("Arial","",9);
					 ...
					 //CODTIPAPO
						...
						$this->cell(30,10,number_format($tb->fields["cf_monretencion"],2,',','.'),0,0,'R');
							...
							$tb1=$this->bd->select("SELECT coalesce(SUM(MONTO),0) as ELMONTO FROM NPHISCON A, NPHOJINT B, NPCONTIPAPORET C  WHERE trim(C.CODTIPAPO)='0004' AND  A.CODNOM=C.CODNOM AND A.CODCON=C.CODCON AND upper(trim(C.TIPO))='A' AND  trim(B.CODEMP)=trim(A.CODEMP) AND  trim(B.CODEMP)=trim('".$tb->fields["codemp"]."') AND A.FECNOM >= to_date('".$this->fechades."','dd/mm/yyyy') AND A.FECNOM <= to_date('".$this->fechahas."','dd/mm/yyyy')");
