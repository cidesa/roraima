<?
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->autorizado=$_POST["autorizado"];//P32-P33-P34-P35-P36-P21-P22-P23-P24-P25
		      $this->especial=$_POST["especial"];
					if($this->especial=="N"){
					$especial="(G.especial IS NULL OR  G.especial='N')  and"; //$especial="G.especial<>'S'   and"
		...
		function Header()
		{
			...
			$this->cell(100,5,'PERIODO DEL: '.$fechad.' AL '.$fechah);
			$this->setTextColor(0,0,255);
			$this->ln(5);
		...
		function Cuerpo()
		{
			...
			if(!$tb2->EOF)
			{
				 ...
				 $this->cell(20,5,$tb->fields["cedemp"]);
				 $this->setFont("Arial","",7);
				 $this->cell(50,5,substr($tb->fields["nomemp"],0,32));
				 $this->cell(60,5,substr($tb->fields["nomcar"],0,40));
			...
			while(!$tb->EOF)
			{
				if($tb->fields["codemp"]!=$ref)
				{
				 ...
				 $this->cell(30,5,number_format($acum4,2,'.',','));
				 ...
				 $this->line(10,$this->getY(),270,$this->getY());
				 $this->ln(2);
				 $this->cell(130,5,'TOTAL NOMINA:  '.$this->nombre);
				 $this->cell(30,5,number_format($totacum1,2,'.',','));
				 $this->cell(30,5,number_format($totacum2,2,'.',','));
				 $this->cell(30,5,number_format($totacum4,2,'.',','));
