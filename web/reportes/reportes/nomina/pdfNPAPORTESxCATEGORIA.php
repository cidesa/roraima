<?
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->sql="select distinct(a.codtipapo),a.destipapo,a.porapo,a.porret,b.codcon as codconapo,b.codnom as codnomapo , 
						c.codpar,d.nompar 
						from nptipaportes a,npcontipaporet b, npdefcpt c,nppartidas d
						where 
						a.codtipapo=b.codtipapo and 
						a.codtipapo>='".$this->apor1."' and a.codtipapo<='".$this->apor2."' and
						b.codnom =('".$this->nom."') and 
						tipo='A' and c.codcon=b.codcon and
						c.codpar=d.codpar
		...
		function Cuerpo()
		{
			...
			$this->sqlc="select a.codtipapo,a.destipapo,a.porapo,a.porret,b.codcon as codconapo,b.codnom as codnomapo , 
						c.codpar,d.nompar 
						from nptipaportes a,npcontipaporet b, npdefcpt c,nppartidas d
						where 
						a.codtipapo=b.codtipapo and 
			...
			while (!$tb->EOF)
			{
				...
				$this->sqlx="select distinct (c.codcat) as codcat,a.codnom,d.nomcat
							from npnomcal a, npasicaremp c, npcatpre d,
							nphojint b
							where
							
							a.codemp=c.codemp and a.codcar=c.codcar and a.codnom=c.codnom and
							b.codemp=a.codemp and
							
							c.codcat=d.codcat and a.codnom=c.codnom and
							a.codnom='".$codnom."' and a.codcon='".$codcon."' and
							c.codcat>=  rpad('".$this->cat1."',16,' ') and  c.codcat<= rpad('".$this->cat2."',16,' ')
							order by d.nomcat,c.codcat
							";
				...
				$this->ln(7);
				$this->cell(10,5,"");
				$this->cell(25,5,$tbx->fields["codcat"]);
				$this->cell(50,5,strtoupper($tbx->fields["nomcat"]));
				$this->ln(4);				
				$this->cell(10,5,"");
				$this->cell(25,5,$tb->fields["codpar"]);
				$this->cell(50,5,strtoupper($tb->fields["nompar"]));
				...
				$codcat=$tbx->fields["codcat"];
				
				$this->sql2="select distinct(a.codemp) as codemp,a.codcar,
						    sum(a.saldo) as monto,
							to_char(coalesce(fecrei,fecing),'dd/mm/yyyy') as fecing,b.nomemp
							from npnomcal a,nphojint b, npasicaremp c, npcatpre d
							where
							c.codcat='".$codcat."' and
							a.codemp=c.codemp and a.codcar=c.codcar and a.codnom=c.codnom and
							b.codemp=a.codemp and
							a.codnom='".$codnom."' 
							and a.codcon='".$codcon."' 
							and b.codemp >=  rpad('".$this->emp1."',16,' ') and  b.codemp<= rpad('".$this->emp2."',16,' ')
							group by a.codemp,a.codnom,a.codcon,a.codcar,b.nomemp,coalesce(fecrei,fecing),c.codcat,d.nomcat,a.saldo
							order by b.nomemp,a.codemp";
				...
				$this->sqlb="select coalesce(sum(monto),0) as valor 
							...
							and a.codcon=b.codcon and codnom='".$this->nom."'";
