...
/*
	function CF_SALDOOPFormula return Number is

MONTO_CIERRE NUMBER(16,2);
MONTO_OPRET NUMBER(16,2);
begin
  SELECT NVL(SUM(MONTO),0) INTO MONTO_CIERRE 
  FROM NPCIENOM
  WHERE CODPRE=RPAD(:CODPRE,32,' ')AND
  (ASIDED='A' OR ASIDED='P');
  
  SELECT NVL(SUM(B.MONRET),0) INTO MONTO_OPRET   
  FROM OPORDPAG A,OPRETORD B 
  WHERE A.NUMORD = B.NUMORD AND 
  B.NUMRET = 'NOASIGNA'  AND 
  A.TIPCAU = 'OP/N' AND
  B.CODPRE=RPAD(:CODPRE,32,' ');
  
  RETURN(MONTO_CIERRE+MONTO_OPRET);
  RETURN NULL;
EXCEPTION WHEN NO_DATA_FOUND THEN
	RETURN(0);
				...
				*/
				...
				$this->setFont("Arial","",8); 
				 ...
				 $this->cell($this->anchos[3],7,$tb2g->fields["saldo_dis"]);				 				 
   	 			 ...
   	 			 $this->sql8="SELECT coalesce(SUM(MONTO),0) as monto_cierre FROM NPCIENOM WHERE RPAD(CODPRE,32,' ')=RPAD('".$tb->fields["codpre"]."',32,' ') AND (ASIDED='A' OR ASIDED='P')";
   	 			 				 
			     $tb2h=$this->bd->select($this->sql8);				 
				 
   	 			 $this->sql9="SELECT coalesce(SUM(B.MONRET),0) as monto_opret FROM OPORDPAG A,OPRETORD B WHERE A.NUMORD = B.NUMORD AND B.NUMRET = 'NOASIGNA' 
			...
			$this->ln();
			$this->Setx(22);
			$this->cell(50,5,$this->p_e); 
			$this->Setx(84);
			$this->cell(50,5,$this->p_a);
			$this->Setx(155);
			$this->cell(50,5,$this->p_r);
			$this->Setx(225);
			$this->cell(50,5,$this->p_conf);
			$this->ln();
			$this->ln();
