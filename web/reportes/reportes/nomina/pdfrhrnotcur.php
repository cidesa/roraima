	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->sql="SELECT 
							...
							B.NOTCUR as notcur,
							C.NOMEMP as nomemp,
							(CASE WHEN B.APRCUR='s' THEN 'SI' WHEN B.APRCUR='n' THEN 'NO' ELSE '' END) as aprcur
						FROM RHDEFCUR A, RHNOTCUR B,NPHOJINT C
						WHERE 
		...
		function Cuerpo()
		{
			...
			while(!$tb->EOF)
			{
				if($tb->fields["codcur"]!=$ref)
				{
				 ...
				 $this->cell($this->anchos[4],10,$tb->fields["notcur"]);
				 $this->cell($this->anchos[5],10,$tb->fields["aprcur"]);
