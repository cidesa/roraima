        ...
        <div align="left">&nbsp; 
          <table width="612"  border="0" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            <tr bordercolor="#FFFFFF"> 
              <td colspan="3" class="form_label_01"> <div align="center"> <font color="#000066" size="4"><strong>RESUMEN DE CONCEPTOS
           ...
           <tr>
			...
			<tr>
              ...
              <td> <div align="left"> </div>
                <div align="left"> 
			  	...
			  	$tb=$bd->select("SELECT to_char(MIN(fecemi),'DD/MM/YYYY') as fecha FROM tscheemi");
				if(!$tb->EOF)
				{
					$fecini=$tb->fields["fecha"];
				}
	  	        
			  	$tb2=$bd->select("SELECT to_char(MAX(fecemi),'DD/MM/YYYY') as fecha FROM tscheemi");
				if(!$tb2->EOF)
				{
			...
			<tr>
              ...
              <td><div align="left">
				...
				$sql="select SUBSTR(RTRIM(CASE WHEN D.CODCAT is null then B.CODCAT ELSE D.CODCAT END)||'-'||RTRIM(C.CODPAR)||' ',1,50) as codpre from NPNOMCAL as A left outer join NPCONCEPTOSCATEGORIA as D on (A.CODCON=D.CODCON),
				NPASICAREMP B, (SELECT Z.CODCON,Z.ORDPAG,Z.OPECON,Z.IMPCPT, coalesce(Y.CODPRE,Z.CODPAR) as codpar, coalesce(Y.CODNOM,'XXX') as codnom
 				FROM NPDEFCPT Z left outer join NPASICODPRE Y on (Z.CODCON=Y.CODCON) ORDER BY OPECON,Z.CODCON ) as C ";
                LlenarTextoSql($sql,"codpre",$bd);				
				?>" class="breadcrumb" id="codpredes" size="20" maxlength="50">
                <input type="button" name="Button3" value="..." onClick="catalogo4('codpredes');">
              </div></td>
              <td><input name="codprehas" type="text" value="<?
				$sql="select max(codpre) as codpre FROM CPASIINI WHERE PERPRE='00' ";
                LlenarTextoSql($sql,"codpre",$bd);				
				?>" class="breadcrumb" id="codprehas" size="20" maxlength="50">
              <input type="button" name="Button2" value="..." onClick="catalogo4('codprehas');"></td>
            </tr>
...
{
	f=document.form1;
	f.titulo.value="RESUMEN DE CONCEPTOS";
	f.action="rNPRNOMPRE1.php";
	f.submit();
}
function cerrar()
{
...
function catalogo3(campo)
   {
      mysql='SELECT DISTINCT(CODNOM) as Codigo,nomnom as Nombre FROM NPNOMCAL ORDER BY CODNOM';
	  pagina="../../lib/general/catalogoobj.php?sql="+mysql+"&campo="+campo;
	  window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,resizable=yes,left=50,top=50");
   }    
...
function catalogo4(campo)
   {
      mysql='select SUBSTR(RTRIM(CASE WHEN D.CODCAT is null then B.CODCAT ELSE D.CODCAT END)||?-?||RTRIM(C.CODPAR)||? ?,1,50) as codpre from NPNOMCAL as A left outer join NPCONCEPTOSCATEGORIA as D on (A.CODCON=D.CODCON),
				NPASICAREMP B, (SELECT Z.CODCON,Z.ORDPAG,Z.OPECON,Z.IMPCPT, coalesce(Y.CODPRE,Z.CODPAR) as codpar, coalesce(Y.CODNOM,?XXX?) as codnom
 				FROM NPDEFCPT Z left outer join NPASICODPRE Y on (Z.CODCON=Y.CODCON) ORDER BY OPECON,Z.CODCON ) as C ' ;
	  pagina="../../lib/general/catalogoobj.php?sql="+mysql+"&campo="+campo;
	  window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,resizable=yes,left=50,top=50");
