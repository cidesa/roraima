        ...
        <div align="left">&nbsp;
          <table width="612"  border="0" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            <tr bordercolor="#FFFFFF">
              <td colspan="3" class="form_label_01"> <div align="center"> <font color="#000066" size="4"><strong>FICHA DE BENEFICIARIOS
                  DE FONDOS EN AVANCE
            ...
            <tr bordercolor="#6699CC">
              <td class="form_label_01"><strong>C&oacute;digo del Beneficiario:</strong></td>
              <td>
                <?

			  	$tb=$bd->select("SELECT min(CEDRIF) as cedrif FROM OPBENFONAVA");
               ?>
                <input name="cedrif1" type="text" value="<?print $tb->fields["cedrif"];

			  ?>"class="breadcrumb" id="cedrif1" size="16" maxlength="100">

         				<input type="button" name="Button" value="..." onClick="catalogo1('cedrif1');">
         		</td>
            <td>
                <?

			  	$tb=$bd->select("SELECT max(CEDRIF) as cedrif FROM OPBENFONAVA");
               ?>
                <input name="cedrif2" type="text" value="<?print $tb->fields["cedrif"];

			  ?>"class="breadcrumb" id="cedrif2" size="16" maxlength="100">

         				<input type="button" name="Button" value="..." onClick="catalogo1('cedrif2');">
		</td>
            </tr>
              ...
              <td
                <?
			  	...
			  	$tb=$bd->select("SELECT min(A.NOMBEN) as nomben FROM OPBENEFI A, OPBENFONAVA B WHERE A.CEDRIF=B.CEDRIF");
               ?>
...
{
	f=document.form1;
	f.titulo.value="FICHA DE BENEFICIARIOS DE FONDOS EN AVANCE";
	f.action="rtsrbenfonava.php";
	f.submit();
}
function cerrar()
{
...
{
    pagina="../../lib/general/catalogoobj.php?campo="+campo+"&sql=SELECT distinct(CEDRIF) as cedrif FROM OPBENFONAVA order by cedrif";
...
{
    pagina="../../lib/general/catalogoobj.php?campo="+campo+"&sql=SELECT distinct(A.NOMBEN) as nomben FROM OPBENEFI A, OPBENFONAVA B WHERE A.CEDRIF=B.CEDRIF order by nomben";
	window.open(pagina,"Catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,heigth=400,resizable=yes,left=50,top=50");
}
</script>
<?$bd->closed()?>
