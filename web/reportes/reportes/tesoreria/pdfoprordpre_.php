...
//Definiciones de Funciones
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			if($_GET["ordpagdes"]!="")
			{
				...
				$this->arrp=$this->oprordpre->sqlp1($this->ordpagdes,$this->ordpaghas);
			}
			else
			{
			//Recibir las variables por el Post
			$this->ordpagdes=H::GetPost("ordpagdes");
			$this->ordpaghas=H::GetPost("ordpaghas");
			...
			$this->oprordpre= new Oprordpre();
	        $this->arrp=$this->oprordpre->sqlp2($this->ordpagdes,$this->ordpaghas,$this->bendes,$this->benhas,$this->fechades,$this->fechahas,$this->tipcaudes,$this->tipcauhas);
			}
	...
	//	$this->Cell(20,4,'NETO A PAGAR Bs.',0,0,'R');
		...
		$this->line(155,251, 205, 251);
		$this->SetXY(160, 251);
		$this->Cell(20, 5, 'CONTRALOR MUNICIPAL (E)');
		}
		...
		{$i=0;
			...
			{
				$tipcau=$this->arrp[$this->i]["tipcau"];
				$eof=count($this->arrp);
				$i++;
				$this->SetXY(170,26);
				...
				$this->SetXY(12, 49);
				$this->MultiCell(100,4,$this->arrp[$this->i]["nomben"],0,'J',0);
				$this->SetXY(166,50);
				...
				$montototal=$this->arrp[$this->i]["monord"]-$this->arrp[$this->i]["monret"];
				$this->SetXY(10,183);
				...
				if($this->arrp[$this->i]["orden"]!=$this->pase)
				{
                    ...
                    $p=count($this->tb3);
					 $this->sql4="select b.codpre , ".$misql.", sum(b.moncau) as moncau ,b.monret as monret,RTRIM(C.NOMPRE) as nompre, substr(a.fecemi,1,4) as fecemi
					 	       from opordpag as a,OPDETORD as b, CPDEFTIT as c
                                  			WHERE B.NUMORD = ('".$this->arrp[$this->i]["orden"]."')
								and b.CODPRE = C.CODPRE
								and a.numord = b.numord
							group by b.codpre,b.monret,c.nompre,a.fecemi
							order by b.codpre";
                            	//print $this->sql4; exit;
                     $this->tb4=$this->bd->select($this->sql4);

					 $this->sql5="select d.codtip,sum(d.monret) as monret, b.destip,a.tipcau
									from opretord d, opordpag a, optipret b
									where
									a.NUMORD = ('".$this->arrp[$this->i]["orden"]."') and
									a.numord=d.numord and
									b.codtip=d.codtip
									group by d.codtip, b.destip,a.tipcau
									order by d.codtip";
...
/////////////////////////////////
				while(!$this->tb4->EOF)
				{
							...
							$this->SetX(72);
							$this->cell(10,4,$this->tb4->fields[7],0,0,'C');
							...
							$this->setFont("Arial","",7);
							$this->SetX(82);
							$this->multiCell(85,4,strtoupper($this->tb4->fields["nompre"]));
							  if ($this->getY()>=145)//agregar pagina si el detalle pasa de getY 180
									{
										$this-> AddPage();
										$this->SetXY(170,26);
										$this->SetFont("Arial", "B", 9);
...
//echo $tipo;
	...
	$entro=false;
					 while(!$this->tb5->EOF)
					 {
						...
						if($this->tb5->fields["tipcau"]=='CPNO')
						{
						 $this->SetX(185);
							...
							if ($this->getY()>168){
								...
								//total
								 ...
								 if($tipcau=='C/PE')
								 {
									...
									$this->SetXY(185,151);
								 	$this->Cell(20,3,number_format($montototal,2,',','.'),0,0,'R');
						...
						$this->SetX(145);
					    $this->Cell(20,3,($this->tb5->fields["destip"]),0,0,'R');
						$this->SetX(185);
					...
					/* $this->SetXY(140,130);
	                 ...
	                 //total
	                 //si entro a la condicion de si gety es mayor a 168 no imprime porq ya lo imprimio en la condicion de arriba
	              if (!$entro){
					 if($tipcau=='C/PE')
					 {
						$this->SetXY(185,151);
