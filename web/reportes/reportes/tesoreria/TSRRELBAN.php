        ...
        <div align="left">&nbsp; 
          <table width="612"  border="0" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            ...
            <tr> 
			  ...
			  ?>
                <select name="codban1" class="breadcrumb" id="codban1">
                  <?
				  	while(!$tb->EOF)	
					{
				  ?>
              ...
              <td> 
                <?
			  	...
			  	$tb=$bd->select("SELECT DISTINCT NUMCUE FROM TSDEFBAN ORDER BY NUMCUE DESC");
			  ?>
                <select name="codban2" class="breadcrumb" id="codban2">
                  <?
				  	while(!$tb->EOF)	
					{
				  ?>
            ...
            <tr bordercolor="#6699CC"> 
              <td class="form_label_01"><strong>Tipo Cuenta:</strong></td>
              <td> 
                <?
			  	
			  	$tb=$bd->select("SELECT DISTINCT CODTIP as cod, DESTIP as des FROM TSTIPCUE ORDER BY CODTIP");
			  ?>
                <select name="tipcue1" class="breadcrumb" id="tipcue1">
                  <?
				  	while(!$tb->EOF)	
					{
				  ?>
                  <option value="<? print $tb->fields["cod"];?>"><? print $tb->fields["des"];?></option>
                  <?
										$tb->MoveNext();
					}
              ...
              <td> 
                <?
			  	...
			  	$tb=$bd->select("SELECT DISTINCT CODTIP as cod, DESTIP as des FROM TSTIPCUE ORDER BY CODTIP DESC");
			  ?>
                <select name="tipcue2" class="breadcrumb" id="tipcue2">
                  <?
				  	while(!$tb->EOF)	
					{
				  ?>
            ...
            <tr bordercolor="#6699CC"> 
              <td class="form_label_01"><strong>Saldos Mayor o Igual a:</strong></td>
              <td><select name="select" class="breadcrumb" id="select">
            ...
            <tr bordercolor="#6699CC"> 
              ...
              <td> <div align="left"> </div>
                <div align="left"> 
			  	...
			  	$tb=$bd->select("SELECT to_char(MIN(fecini),'DD/MM/YYYY') as fecha FROM cppereje");
				if(!$tb->EOF)
				{
					$fecini=$tb->fields["fecha"];
				}
	  	        
			  	$tb2=$bd->select("SELECT to_char(MAX(fecini),'DD/MM/YYYY') as fecha FROM cppereje");
				if(!$tb2->EOF)
				{
...
{
	f=document.form1;
	f.titulo.value="RELACION DE CUENTAS BANCARIAS";
