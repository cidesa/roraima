          ...
          <table width="612"  border="0" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            ...
            <tr bordercolor="#6699CC">
              ...
              <td>
                <?
			  	...
			  	$tb=$bd->select("SELECT min(NUMORD) as numord FROM OPORDPAG WHERE STATUS='I'");
               ?>
            ...
            <td>
                <?
			  	...
			  	$tb=$bd->select("SELECT max(NUMORD) as numord FROM OPORDPAG WHERE STATUS='I'");
               ?>
            ...
            <tr bordercolor="#6699CC">
              ...
              <td>
                <?
			  	...
			  	$tb=$bd->select("SELECT min(CODUBI) as codubi FROM BNUBICA WHERE LENGTH(TRIM(CODUBI))=11");
               ?>
                <input name="codubi1" type="text" value="<?print $tb->fields["codubi"];

			  ?>"class="breadcrumb" id="codubi1" size="16" maxlength="100">

         				<input type="button" name="Button" value="..." onClick="catalogo3('codubi1');">
         		</td>
            <td>
                <?

			  	$tb=$bd->select("SELECT max(CODUBI) as codubi FROM BNUBICA WHERE LENGTH(TRIM(CODUBI))=11");
               ?>
                <input name="codubi2" type="text" value="<?print $tb->fields["codubi"];

			  ?>"class="breadcrumb" id="codubi2" size="16" maxlength="100">

         				<input type="button" name="Button" value="..." onClick="catalogo3('codubi2');">
			</td>
            </tr>
            ...
            <tr bordercolor="#6699CC">
              ...
              <td> <div align="left"> </div>
                <div align="left">
			  	...
			  	$tb=$bd->select("SELECT to_char(MIN(fecemi),'DD/MM/YYYY') as fecha FROM OPORDPAG WHERE STATUS='I' ");
				if(!$tb->EOF)
				{
					$fecini=$tb->fields["fecha"];
				}

			  	$tb2=$bd->select("SELECT to_char(MAX(fecemi),'DD/MM/YYYY') as fecha FROM OPORDPAG WHERE STATUS='I'");
				if(!$tb2->EOF)
				{
            ...
            <tr bordercolor="#6699CC">
			  ...
			  ?>"class="breadcrumb" id="numcue1" size="16" maxlength="100">
         				...
         				<input type="button" name="Button" value="..." onClick="catalogo4('numcue1');">
         		</td>
            <td>
                <?
			  ...
			  ?>"class="breadcrumb" id="numcue2" size="16" maxlength="100">
         				...
         				<input type="button" name="Button" value="..." onClick="catalogo4('numcue2');">
			</td>
            </tr>
            <tr>
              <td class="form_label_01">&nbsp;</td>
...
{
	f=document.form1;
	f.titulo.value="RELACION DE ORDENES DE PAGO CANCELADAS";
	f.action="rtsrlisordpagcan.php";
	f.submit();
}
function cerrar()
{
...
{
    pagina="../../lib/general/catalogoobj.php?campo="+campo+"&sql=SELECT distinct(CEDRIF) as cedrif,NOMBEN FROM OPBENEFI ORDER BY CEDRIF";
	window.open(pagina,"Catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,heigth=400,resizable=yes,left=50,top=50");
}
function catalogo3(campo)
{
    pagina="../../lib/general/catalogoobj.php?campo="+campo+"&sql=SELECT DISTINCT(CODUBI) as codubi,DESUBI FROM BNUBICA WHERE LENGTH(TRIM(CODUBI))=11 ORDER BY CODUBI";
	window.open(pagina,"Catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,heigth=400,resizable=yes,left=50,top=50");
}
