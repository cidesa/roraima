	...
	class pdfreporte extends fpdf
	{
		...
		function Header()
		{
			...
			*/
			...
			$this->setFont("Arial","B",9);
    		$this->Ln(4);
    		$this->Cell(210,5,'REPUBLICA BOLIVARIANA DE VENEZUELA',0,0,'C');
			$this->Ln(5);
...
A.RIFPRO  >=  :RIFPRODES AND
A.RIFPRO  <=  :RIFPROHAS AND
A.NOMPRO>= :NOMPRODES AND
A.NOMPRO<= :NOMPROHAS
ORDER BY A.CODPRO
		...
		*/
			...
			$this->sql1= "SELECT DISTINCT A.REFCAU, A.DESCAU, A.FECCAU, A.MONCAU as moncau, B.NOMBEN as nomben
						...
						ORDER BY A.REFCAU";
				//ojo quite A.CEDRIF=B.CEDRIF esto de where
				//ojo esta sacando uno solo lo q cambia es el nombre del beneficiario :s
			$tb1=$this->bd->select($this->sql1);


		while(!$tb1->EOF) {

			$this->Cell(180,5,'AUTORIZACIÓN DE PAGO No.: '.$this->numau,0,0,'R');
			$this->Ln(5);
			...
			$this->Ln(5);
			$this->Cell(210,5,'				Fondo de Fideicomiso, el pago por concepto de:',0,0,'L');
			$this->Ln(15);
