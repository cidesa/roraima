        ...
        <div align="left">&nbsp; 
          <table width="647"  border="1" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            ...
            <tr>
			  ...
			  <?
			  	$h=$bd->select("SELECT DISTINCT(to_char(FECNOM,'dd/mm/yyyy')) as fecnom FROM NPHISCON ORDER BY FECNOM DESC");
			  ?>
              <td><select name="fechades" class="breadcrumb" id="fechades">
			  <?
			  	while(!$h->EOF)
				{
			  ?>
			  <option value="<? print $h->fields["fecnom"];?>"><? print $h->fields["fecnom"];?></option>
			  <?
			   	$h->MoveNext();
			   }	
			  ?>
			  ...
			  <?
			  	$c=$bd->select("SELECT DISTINCT(to_char(FECNOM,'dd/mm/yyyy')) as fecnom FROM NPHISCON ORDER BY FECNOM DESC");
			  ?>
              <td><select name="fechahas" class="breadcrumb" id="fechahas">
			  <?
			  	while(!$c->EOF)
				{
			  ?>
			  <option value="<? print $c->fields["fecnom"];?>"><? print $c->fields["fecnom"];?></option>
			  <?
			   	$c->MoveNext();
			   }	
			  ?>
