					...
					$this->setFont("Arial","",9);
					 ...
					 //CODTIPAPO
						...
						$this->cell(30,10,number_format($tb->fields["cf_monretencion"],2,'.',','),0,0,'R');
							...
							$tb1=$this->bd->select("SELECT coalesce(SUM(SALDO),0) as ELMONTO FROM NPNOMCAL A, NPHOJINT B, NPCONTIPAPORET C  WHERE rtrim(C.CODTIPAPO)='0004' AND  A.CODNOM=C.CODNOM AND A.CODCON=C.CODCON AND upper(trim(C.TIPO))='A' AND  B.CODEMP=A.CODEMP AND  trim(B.CODEMP)='".$tb->fields["codemp"]."'");		
