<?
	...
	class pdfreporte extends fpdf
	{
		...
		function Header()
		{
			...
			$this->Ln(18);
			$this->Cell(10);
			$this->Cell(88,4,"No. PAGADO: ".$this->tb[$this->indice]["refpag"],0,0,'L');
			$this->Cell(88,4,"FECHA PAGADO: ".$this->tb[$this->indice]["fecpag"],0,0,'R');
			$this->Ln(9);
			...
			$this->Ln(5);
			$this->Cell(10);
			$this->MultiCell(176,4,trim($this->tb[$this->indice]["despag"]));
		...
		function Cuerpo()
		{
			$ref=$this->tb[$this->indice]["refpag"];
			$posy1=$this->GetY();
			$empieza1=$this->GetY();
			$posy2=170;
			$empieza2=170;
			...
			while(($ref == $this->tb[$this->indice]["refpag"]) and ($this->indice < $this->cuantos))
			{
				$this->SetY($posy1);
				...
				{
					$ctacont=$this->bd->sql_contabb($this->tb[$this->indice]["refpag"],$this->tb[$this->indice]["codcta"]);
				...
				$this->indice++;
				if($posy1>=156 or $posy2>=206 or  $ref!=$this->tb[$this->indice]["refpag"])
				{
					$posy1=$empieza1;
					$posy2=$empieza2;
					if($ref!=$this->tb[$this->indice]["refpag"])
					{
