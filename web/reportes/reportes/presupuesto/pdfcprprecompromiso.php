<?
	...
	require_once("../../lib/modelo/sqls/presupuesto/Cprprecompromiso.class.php");
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->precompdes=str_replace('*',' ',H::GetPost("refpredes"));
			$this->precomphas=str_replace('*',' ',H::GetPost("refprehas"));
			$this->bd=new Cprprecompromiso();
			$this->arrp=$this->bd->sqlp($this->precompdes,$this->precomphas);
			$this->tb=$this->arrp;
		...
		function Header()
		{
			...
			{
				$this->getCabecera(H::GetPost("Comprobante de PreCompromiso"),"");
			}
			...
			$this->Ln(18);
			$this->Cell(10);
			$this->Cell(88,4,"No. PRECOMPROMISO: ".$this->tb[$this->indice]["refprc"],0,0,'L');
			$this->Cell(88,4,"FECHA PRECOMPROMISO: ".$this->tb[$this->indice]["fecprc"],0,0,'R');
			$this->Ln(9);
			...
			$this->Ln(5);
			$this->Cell(10);
			$this->MultiCell(176,4,trim($this->tb[$this->indice]["desprc"]));
		...
		function Cuerpo()
		{
			$ref=$this->tb[$this->indice]["refprc"];
			$posy1=$this->GetY();
			$empieza1=$this->GetY();
			$posy2=170;
			$empieza2=170;
			...
			while(($ref == $this->tb[$this->indice]["refprc"]) and ($this->indice < $this->cuantos))
			{
				$this->SetY($posy1);
				...
				{
					$ctacont=$this->bd->sql_contabb($this->tb[$this->indice]["refprc"],$this->tb[$this->indice]["codcta"]);
				...
				$this->indice++;
				if($posy1>=156 or $posy2>=206 or  $ref!=$this->tb[$this->indice]["refprc"])
				{
					$posy1=$empieza1;
					$posy2=$empieza2;
					if($ref!=$this->tb[$this->indice]["refprc"])
					{
