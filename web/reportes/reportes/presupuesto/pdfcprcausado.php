<?
	...
	require_once("../../lib/modelo/sqls/presupuesto/Cprcausado.class.php");
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->caudes=str_replace('*',' ',H::GetPost("refcaudes"));
			$this->cauhas=str_replace('*',' ',H::GetPost("refcauhas"));
			
			$this->bd=new Cprcausado();
			$this->arrp=$this->bd->sqlp($this->caudes,$this->cauhas);
			$this->tb=$this->arrp;
		...
		function Header()
		{
			...
			{
				$this->cab->poner_cabecera($this,"Comprobante de Compromiso","p","s");
			}
			...
			$this->Ln(18);
			$this->Cell(10);
			$this->Cell(88,4,"No. CAUSADO: ".$this->tb[$this->indice]["refcau"],0,0,'L');
			$this->Cell(88,4,"FECHA CAUSADO: ".$this->tb[$this->indice]["feccau"],0,0,'R');
			$this->Ln(9);
			...
			$this->Ln(5);
			$this->Cell(10);
			$this->MultiCell(176,4,trim($this->tb[$this->indice]["descau"]));
		...
		function Cuerpo()
		{
			$ref=$this->tb[$this->indice]["refcau"];
			$posy1=$this->GetY();
			$empieza1=$this->GetY();
			$posy2=170;
			$empieza2=170;
			...
			while(($ref == $this->tb[$this->indice]["refcau"]) and ($this->indice < $this->cuantos))
			{
				$this->SetY($posy1);
				...
				{
					$ctacont=$this->bd->sql_contabb($this->tb[$this->indice]["refcau"],$this->tb[$this->indice]["codcta"]);
				...
				$this->indice++;
				if($posy1>=156 or $posy2>=206 or  $ref!=$this->tb[$this->indice]["refcau"])
				{
					$posy1=$empieza1;
					$posy2=$empieza2;
					if($ref!=$this->tb[$this->indice]["refcau"])
					{
