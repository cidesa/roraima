...
//print '<pre>'; print $this->sql;exit;
		...
		function Header()
		{
			...
			if(strtoupper(trim($aux[1]))=="SECTOR" && $this->nivhasta=="general")
			{
				$this->titulo.="  Por Sectores";
			}elseif($this->nivhasta=="6" && trim($this->agrupar)=="general")
			{
				$this->titulo.="  Por Partidas";
			}
//$this->agrupar $this->nivhasta
$dig=substr($this->agrupar,0,1);
            if($dig=="1" )
			{
				...
				if($dig=="2" )
			{
				$this->titulod="PROGRAMA";}

				if($dig=="3")
			{
				$this->titulod="ACTIVIDAD";}


				  if($this->nivhasta=="7" )
			{
				$this->titulod2=" - SUB-ESPECIFICA";}
				if($this->nivhasta=="6" )
			{
				$this->titulod2=" - ESPECIFICA";}
				if($this->nivhasta=="5")
			{
				$this->titulod2=" - GENERICA";}

				if($this->nivhasta=="4")
			{
				$this->titulod2=" - PARTIDA";}
			...
			$this->cell(270,4,"EJECUCIÓN PRESUPUESTARIA POR ".$this->titulod.$this->titulod2,0,0,'C');
			$this->ln(4);
				$this->cell(270,4,"(En Bolivares)",0,0,'C');
			$this->setFont("Arial","B",7);
			$this->setY($y);
