<?
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->sqlg="select 
						substr(x.codcat,1,".$this->agrup.") as cat, 
						x.codparegr as par, 
						x.codobr as codorg, 
						sum(x.monpre) as monto
						from						
						forpreobr x, fordefobr y						
						where						
						x.codobr=y.codobr and 
						rtrim(x.codparegr) >= ('".rtrim($this->filtro1)."') and rtrim(x.codparegr) <= ('".rtrim($this->filtro2)."') and
						substr(x.codcat,1,".$this->agrup.") like ('".$this->filtro."')
						
						
						group by substr(x.codcat,1,".$this->agrup."),x.codparegr,x.codobr
						
						order by substr(x.codcat,1,".$this->agrup.")";
		...
		function Cuerpo()
		{
			...
			$sql="Select nomobr as nomorg from fordefobr where codobr = '".$tbg->fields["codorg"]."'";
			$tb=$this->bd->select($sql);
			$this->SetX(32);
			$this->setFont("Arial","",6);
			$this->cell(5,5,substr($tb->fields["nomorg"],0,110));
			$this->setFont("Arial","",7);
			...
			$this->line(175,$this->GetY(),200,$this->GetY());
			$this->SetX(195);
			$this->setFont("Arial","B",7);
			$this->cell(5,5,number_format($acum,2,'.',','),0,0,"R");
		}
