	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			if (trim($this->gasto)=="T")
			{
				$j=0;
					$this->sqlg1="
						select
						substr(codpre,".$this->loncat."+2,".$this->lon2[$j].") as buscar2,sum(monto) as monto
						from forasiini
						where perpre='00'
						and substr(codpre,".$this->loncat."+2,".$this->lon2[$j].") like '".$this->filtro."%'
						group by
						substr(codpre,".$this->loncat."+2,".$this->lon2[$j].")
						having substr(codpre,".$this->loncat."+2,".$this->lon2[$j].") is not null and sum(monto)<>0 ";
				...
				while ($j<$i)
				{
					...
					$this->sqlg2= $this->sqlg2." union 
						select
						substr(codpre,".$this->loncat."+2,".$lon.") as buscar2,sum(monto) as monto
						from forasiini
						where perpre='00'
						and substr(codpre,".$this->loncat."+2,".$lon.") like '".$this->filtro."%'
						group by
						substr(codpre,".$this->loncat."+2,".$lon.")
			...
			else
			{
				$j=0;
					$this->sqlg1="
						...
						substr(codpre,".$this->loncat."+2,".$this->lon2[$j].") as buscar2,sum(monto) as monto
						from forasiini
						where perpre='00' and codtip='".$this->gasto."'
						and substr(codpre,".$this->loncat."+2,".$this->lon2[$j].") like '".$this->filtro."%'
						group by
						substr(codpre,".$this->loncat."+2,".$this->lon2[$j].")
						having substr(codpre,".$this->loncat."+2,".$this->lon2[$j].") is not null and sum(monto)<>0 ";
		...
		function Cuerpo()
		{
			...
			while (!$tbg->EOF)
			{
				...
				$sql="select nomparegr from fordefparegr where trim(codparegr)='".$par."' ";
				$tb=$this->bd->select($sql);
				...
				$this->SetX(40);
				$this->cell(5,5,substr(strtoupper($tb->fields["nomparegr"]),0,80));
