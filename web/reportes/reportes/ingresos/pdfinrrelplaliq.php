<?
	...
	class pdfreporte extends PDF_MC_Table
	{
		...
		function Header()
		{
			...
			$this->Rect(54,$aqui,61,14,'DF');
			$this->Rect(115,$aqui,74,14,'DF');
			$this->cuadros(115,$aqui+5,37,9,2,1,'DF');
			$this->Rect(189,$aqui,40,14,'DF');
		...
		function Cuerpo()
		{
			$this->SetWidths(array(22,22,61,37,37,40,40));
			$this->SetAligns(array('C','C','L','R','R','C','C'));
			$acumulador1=0;
			$acumulador2=0;
			while(!$this->tbg->EOF)
			{
				$this->Row(array($this->tbg->fields["refing"],$this->tbg->fields["fecha"],ucfirst(strtolower($this->tbg->fields["desing"])),number_format($this->tbg->fields["ordinario"],2,'.',','),number_format($this->tbg->fields["coordinado"],2,'.',','),"BANFOANDES",$this->tbg->fields["doc"]));
				$acumulador1+=$this->tbg->fields["ordinario"];
				$acumulador2+=$this->tbg->fields["coordinado"];
				$this->tbg->MoveNext();
			}
			$this->Cell(105,8,"VAN...",1,0,'C');
