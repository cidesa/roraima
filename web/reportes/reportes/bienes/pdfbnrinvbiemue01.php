<?
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->Bnrinvbiemue01 = new Bnrinvbiemue01();
		    $this->arrp=$this->Bnrinvbiemue01->sqlp($this->codactdes, $this->codacthas,$this->codmuedes,$this->codmuehas,$this->fecdes,$this->fechas,$this->codubides,$this->codubihas);
		...
		function Header()
		{
		    ...
		    $this->SetXY(150,10);
		    $this->Cell(120,5,'FORMULARIO BM-1',0,0,'R');
		    $this->SetXY(150,15);
		    ...
		    $this->setFont("Arial","",10);
	  		$this->Cell(35,5,"MUNICIPAL",0,0,'l');
			...
			foreach($this->arrp as $arrp)
			{
				if ($acodubi!=$arrp["acodubi"])
				{
					...
					if ($acodubi)   //Fin de la linea del tipo del Mueble
					{
				 	...
				 	$this->arrp2=$this->Bnrinvbiemue01->sqlp2($arrp["acodubi"]);
...
$this->setBorder(1);
 			//	 $this->Row(array(substr($arrp["acodact"],0,1)."            ".substr($arrp["acodact"],2,2)."            ".substr($arrp["acodact"],3,1),$arrp["acodmue"],$descrip,H::FormatoMonto($arrp["avalini"]),H::FormatoMonto($arrp["avalini"])));
 				$this->Row(array($arrp["acodact"],"1",$arrp["acodmue"],$descrip,H::FormatoMonto($arrp["avalini"]),H::FormatoMonto($arrp["avalini"])));
