...
from CPAsiIni where  (CodPre) =  ('".$codpre."') and AnoPre='2008' and PerPre='00' and a.codpre like  (('".$filtro."'))";
  ...
  {
  	$sql="select
          (case when identi='ADI' then 0
		  when identi='TRA' then 1
		  when identi='DIM' then 2
		  when identi='PRC' then 3
		  when identi='COM' then 4
		  when identi='CAU' then 5
		  when identi='PAG' then 6
		  when identi='AJU' then 7
		  when identi='ANU' then 8 END) as orden,
		  codpre as codpre,
		  nompre as nompre,
