...
class licaclaraActions extends autolicaclaraActions
{
  ...
  {
    if($clasemodelo->getNumacla()=='########')
    {
        $c = new Criteria();
        $per = LidefempPeer::doSelectOne($c);
        $numero = str_pad($per->getAclara(),8,'0',STR_PAD_LEFT);
        $val = H::GetX('Numacla','Liaclara','Numacla',$numero);
        if($val==$numero)
            return 'V008';
        $clasemodelo->setNumacla($numero);
        $sql="update lidefemp set aclara='".($per->getAclara()+1)."'";
