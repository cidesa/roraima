 ...
 * @version SVN: $Id: actions.class.php 33666 2009-10-01 16:36:48Z aperez $
 *
...
class confincieActions extends autoconfincieActions
{
  ...
  public function executeIndex(){
      return $this->redirect('confincie/edit');
  }
  ...
  public function editing()
  {
	...
	$this->contaba = ContabaPeer::doSelectOne(new Criteria());
  ...
  public function executeBtntrasladar()
  {
	$this->contaba = $this->getContabaOrCreate();
    $this->updateContabaFromRequest();
	$res = Contabilidad::trasladarSaldos($this->contaba);
  ...
  public function executeAjaxcomprobante(){
                ...
                $res=Contabilidad::generarComprobantes($this->contaba,&$comprobante);
				if($res==-1){
	                  $concom = count($comprobante);
	                  $form = "sf_admin/confincie/confincomgen";
	                  $i    = 0;
	                  while ($i < $concom){
	                     $f[$i] = $form.$i;
					...
					$this->msgerr=H::obtenerMensajeError($res);
       			}
	  $output =  '[["javascript","",""]]';
      $this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');
