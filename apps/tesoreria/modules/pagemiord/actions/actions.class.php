        ...
        case 'A':
          ...
          if ($tmot=='S') {
            if ($this->opordpag->getFecanu()!="") {
              ...
              $this->eti="ANULADA por el usuario".$nombre;
            }
          }else {
            if ($this->opordpag->getFecanu()!="") {
...
group by numret,a.codtip,b.destip,b.basimp,b.porret,b.factor,b.porsus,b.unitri,c.codrep,c.codret,d.codret";
  ...
  public function setVars() {
    ...
    OrdendePago::formload(&$afectarecargo,&$ordpagnom,&$ordpagapo,&$ordpagliq,&$ordpagfid,&$ordpagval,&$compadic,&$genctaord,&$ordpagcre, &$ordpagsolpag);
            ...
            if ($dato) {
              if ($dato->getGenctaord()=='S') {
                ...
                if (OrdendePago::verificarStatusComprobante($numcom)) {
                  ...
                  $this->msg="No se puede anular la Orden, ya que el Comprobante de Orden asociado esta actualizado";
                  return sfView::SUCCESS;
                }
              }
            }
      ...
      if ($datosret) {
        ...
        if ($data) {
          if (($data->getNumche()=='') || (strlen($data->getNumche())==0)) {
            ...
            return $this->msj="La Orden ya fue pagada en el Módulo de Bancos";
          }
        }//  if ($data)
      }// if ($datosret)
      else {
    ...
    else //orden de pago normal
    {
      ...
      if ($opordpag) {
        if (OrdendePago::verificarStatusComprobante($opordpag->getNumcom())) {
          ...
          if ($data) {
                ...
                if ($dato) {
                  if ($dato->getGenctaord()=='S') {
                    if (OrdendePago::verificarStatusComprobante($numero2)) {
                      ...
                      return $this->msj="No se eliminar ya que el comprobante de orden asociado esta actualizado";
                    }
                  }
                }
              }
            if ($opordpag->getNumcomapr()!='') {
              ...
              if ($puedeeliminar) {
                ...
                return $this->msj="La Orden no fue eliminada";
              }
            }else {
              return $this->msj="La Orden ya fue pagada en el Módulo de Bancos";
            }
          ...
          return $this->msj="No se Puede Eliminar la Orden ya que el Comprobante Contable asociado no existe o esta actualizado";
        }
      }//if ($opordpag)
    }//else orden de pago normal
  }
  ...
  protected function getLabels() {
    ...
    if ($etiforma!="") {
      return array(
              'opordpag{numord}' => 'Referencia:',
              'opordpag{fecemi}' => 'Fecha:',
              'opordpag{tipcau}' => 'Tipo:',
              'opordpag{nomext}' => 'Nombre:',
              'opordpag{desord}' => 'Concepto:',
              'opordpag{cedrif}' => 'C.I o R.I.F:',
              'opordpag{nomben}' => 'Beneficiario:',
              'opordpag{codctasec}' => 'Tipo:',
              'opordpag{ctapag}' => 'Codigo Contable:',
              'opordpag{descta}' => 'Descripción:',
              'opordpag{coduni}' => 'Unidad Origen:',
              'opordpag{desubi}' => 'Descripción:',
              'opordpag{tipfin}' => 'Financiamiento:',
              'opordpag{nomext2}' => 'Nombre2:',
              'opordpag{obsord}' => 'Observaciones:',
              'opordpag{monord}' => 'Monto:',
              'opordpag{mondes}' => 'Monto1:',
              'opordpag{monret}' => 'Monto2:',
              'opordpag{numcom}' => 'Número de Comprobante:',
              'opordpag{feccom}' => 'Fecha del Comprobante:',
              'opordpag{status}' => 'Estatus:',
              'opordpag{idrefer}' => 'Id de Comprobante:',
              'opordpag{fecanu}' => 'Fecha de Anulación:',
              'opordpag{desanu}' => 'Descripción:',
              'opordpag{referencias}' => 'Referencias:',
              'opordpag{documento}' => 'Documento:',
              'opordpag{afectapre}' => 'Afectapre:',
              'opordpag{totalcomprobantes}' => 'Totalcomprobantes:',
              'opordpag{cuentarendicion}' => 'Cuentarendicion:',
              'opordpag{vacio}' => 'Vacio:',
              'opordpag{presiono}' => 'Presiono:',
              'opordpag{numsigecof}' => 'Número:',
              'opordpag{fecsigecof}' => 'Fecha:',
              'opordpag{expsigecof}' => 'Expediente:',
              'opordpag{nombensus}' => 'Cesionario Autorizado:',
