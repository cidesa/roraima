...
<?php $opnn .= input_hidden_tag('nomina', '') ?><?php $opnn .= input_hidden_tag('fechanomina', '') ?><?php $opnn .= input_hidden_tag('gasto', '') ?><?php $opnn .= input_hidden_tag('banco', '') ?>
<? $sql="Select distinct((CASE when c.cedemp is null THEN b.nomnom else C.NOMEMP||' ('||B.NOMNOM||')' END)) as nomina,
   ...
   $url=cross_app_link_to('herramientas','catalogobuscar').'/space/catalogo1/objs/tipnom-nomina-fechanomina-gasto-banco/campos/codigo-nomina-fecha-gasto-codban'; ?>
