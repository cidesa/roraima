...
class ingdefparActions extends autoingdefparActions
{
  ...
  {
    $muni = Ingresosfacturacion::getMunicipio($this->getRequestParameter('inparroquia[inestado_id]',''));
    //H::PrintR($muni);
    $this->inparroquia->setMunicipio($muni);
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      case '1':
        ...
        $this->inparroquia = $this->getInparroquiaOrCreate($codigo);
        $this->inparroquia->setMunicipio(Ingresosfacturacion::getMunicipio($codigo));
   ...
   *
   */
  public function saving($inparroquia)
  {
  	  $sql="select max(id) as num from inparroquia";

      if (Herramientas::BuscarDatos($sql,&$result))
		{
		          	//H::printR ($result);
		          	$inparroquia->setCodpar((str_pad($result[0]["num"]+1, 4, '0', STR_PAD_LEFT)));
		 }

    return parent::saving($inparroquia);
