 ...
 */
class ingdefestActions extends autoingdefestActions
{
  ...
  {
  	if ($this->inestado->getId()=="")
  	{
	  	$sql="select max(id) as num from inestado";
	    if (Herramientas::BuscarDatos($sql,&$result))
	    {
		  $this->inestado->setCodedo(str_pad($result[0]["num"]+1, 4, '0', STR_PAD_LEFT));
	    }
	    else
	    {
	      $this->inestado->setCodedo(str_pad(1, 4, '0', STR_PAD_LEFT));
	    }
  	}
  }
   ...
   *
   */
  public function saving($inestado)
  {
    return parent::saving($inestado);
  }
