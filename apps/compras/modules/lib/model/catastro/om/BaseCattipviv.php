...
abstract class BaseCattipviv extends BaseObject  implements Persistent {
  ...
  public function getDestipviv()
  {

    return trim($this->destipviv);
  ...
  public function getId()
  {
	...
	public function setDestipviv($v)
	{

    if ($this->destipviv !== $v) {
        $this->destipviv = $v;
        $this->modifiedColumns[] = CattipvivPeer::DESTIPVIV;
      }
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = CattipvivPeer::ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->destipviv = $rs->getString($startcol + 0);
      ...
      throw new PropelException("Error populating Cattipviv object", $e);
    }
  }
  ...
  public function __call($m, $a)
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			CattipvivPeer::doDelete($this, $con);
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CattipvivPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += CattipvivPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = CattipvivPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = CattipvivPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = CattipvivPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getDestipviv(),
			$keys[1] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setDestipviv($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CattipvivPeer::DESTIPVIV)) $criteria->add(CattipvivPeer::DESTIPVIV, $this->destipviv);
		if ($this->isColumnModified(CattipvivPeer::ID)) $criteria->add(CattipvivPeer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CattipvivPeer::ID, $this->id);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setDestipviv($this->destipviv);
		...
		if ($this->collCatreginms === null) {
			if ($this->isNew()) {
			   $this->collCatreginms = array();
				...
				$criteria->add(CatreginmPeer::CATTIPVIV_ID, $this->getId());
