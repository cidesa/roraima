...
abstract class BaseCatproter extends BaseObject  implements Persistent {
  ...
  public function getDescatproter()
  {

    return trim($this->descatproter);
  ...
  public function getId()
  {
	...
	public function setDescatproter($v)
	{

    if ($this->descatproter !== $v) {
        $this->descatproter = $v;
        $this->modifiedColumns[] = CatproterPeer::DESCATPROTER;
      }
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = CatproterPeer::ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->descatproter = $rs->getString($startcol + 0);
      ...
      throw new PropelException("Error populating Catproter object", $e);
    }
  }
  ...
  public function __call($m, $a)
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			CatproterPeer::doDelete($this, $con);
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CatproterPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += CatproterPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = CatproterPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = CatproterPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = CatproterPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getDescatproter(),
			$keys[1] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setDescatproter($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CatproterPeer::DESCATPROTER)) $criteria->add(CatproterPeer::DESCATPROTER, $this->descatproter);
		if ($this->isColumnModified(CatproterPeer::ID)) $criteria->add(CatproterPeer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CatproterPeer::ID, $this->id);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setDescatproter($this->descatproter);
		...
		if ($this->collCatreginms === null) {
			if ($this->isNew()) {
			   $this->collCatreginms = array();
				...
				$criteria->add(CatreginmPeer::CATPROTER_ID, $this->getId());
