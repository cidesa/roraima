...
abstract class BaseTabla53 extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->refpag !== $v) {
        $this->refpag = $v;
        $this->modifiedColumns[] = Tabla53Peer::REFPAG;
      }
    ...
    if ($this->fecpag !== $ts) {
      $this->fecpag = $ts;
      $this->modifiedColumns[] = Tabla53Peer::FECPAG;
    }
    ...
    if ($this->refcom !== $v) {
        $this->refcom = $v;
        $this->modifiedColumns[] = Tabla53Peer::REFCOM;
      }
    ...
    if ($this->feccom !== $ts) {
      $this->feccom = $ts;
      $this->modifiedColumns[] = Tabla53Peer::FECCOM;
    }
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = Tabla53Peer::ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      throw new PropelException("Error populating Tabla53 object", $e);
    }
  }
  ...
  public function __call($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = Tabla53Peer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += Tabla53Peer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Tabla53Peer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Tabla53Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = Tabla53Peer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Tabla53Peer::REFPAG)) $criteria->add(Tabla53Peer::REFPAG, $this->refpag);
		if ($this->isColumnModified(Tabla53Peer::FECPAG)) $criteria->add(Tabla53Peer::FECPAG, $this->fecpag);
		if ($this->isColumnModified(Tabla53Peer::REFCOM)) $criteria->add(Tabla53Peer::REFCOM, $this->refcom);
		if ($this->isColumnModified(Tabla53Peer::FECCOM)) $criteria->add(Tabla53Peer::FECCOM, $this->feccom);
		if ($this->isColumnModified(Tabla53Peer::ID)) $criteria->add(Tabla53Peer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(Tabla53Peer::ID, $this->id);
