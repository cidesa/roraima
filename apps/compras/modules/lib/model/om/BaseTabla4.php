...
abstract class BaseTabla4 extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->refcau !== $v) {
        $this->refcau = $v;
        $this->modifiedColumns[] = Tabla4Peer::REFCAU;
      }
    ...
    if ($this->codpre !== $v) {
        $this->codpre = $v;
        $this->modifiedColumns[] = Tabla4Peer::CODPRE;
      }
    ...
    if ($this->monimp !== $v) {
        ...
        $this->modifiedColumns[] = Tabla4Peer::MONIMP;
      }
    ...
    if ($this->monpag !== $v) {
        ...
        $this->modifiedColumns[] = Tabla4Peer::MONPAG;
      }
    ...
    if ($this->monaju !== $v) {
        ...
        $this->modifiedColumns[] = Tabla4Peer::MONAJU;
      }
    ...
    if ($this->staimp !== $v) {
        $this->staimp = $v;
        $this->modifiedColumns[] = Tabla4Peer::STAIMP;
      }
    ...
    if ($this->refere !== $v) {
        $this->refere = $v;
        $this->modifiedColumns[] = Tabla4Peer::REFERE;
      }
    ...
    if ($this->refprc !== $v) {
        $this->refprc = $v;
        $this->modifiedColumns[] = Tabla4Peer::REFPRC;
      }
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = Tabla4Peer::ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      throw new PropelException("Error populating Tabla4 object", $e);
    }
  }
  ...
  public function __call($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = Tabla4Peer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += Tabla4Peer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Tabla4Peer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Tabla4Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = Tabla4Peer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Tabla4Peer::REFCAU)) $criteria->add(Tabla4Peer::REFCAU, $this->refcau);
		if ($this->isColumnModified(Tabla4Peer::CODPRE)) $criteria->add(Tabla4Peer::CODPRE, $this->codpre);
		if ($this->isColumnModified(Tabla4Peer::MONIMP)) $criteria->add(Tabla4Peer::MONIMP, $this->monimp);
		if ($this->isColumnModified(Tabla4Peer::MONPAG)) $criteria->add(Tabla4Peer::MONPAG, $this->monpag);
		if ($this->isColumnModified(Tabla4Peer::MONAJU)) $criteria->add(Tabla4Peer::MONAJU, $this->monaju);
		if ($this->isColumnModified(Tabla4Peer::STAIMP)) $criteria->add(Tabla4Peer::STAIMP, $this->staimp);
		if ($this->isColumnModified(Tabla4Peer::REFERE)) $criteria->add(Tabla4Peer::REFERE, $this->refere);
		if ($this->isColumnModified(Tabla4Peer::REFPRC)) $criteria->add(Tabla4Peer::REFPRC, $this->refprc);
		if ($this->isColumnModified(Tabla4Peer::ID)) $criteria->add(Tabla4Peer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(Tabla4Peer::ID, $this->id);
