...
pake_task('propel-build-model-by-file', 'schema_exists');

pake_desc('create sql for current model');
pake_task('propel-build-sql-by-file', 'schema_exists');
...
function _propel_convert_yml_schema_by_file($check_schema = true, $prefix = '', $args = array())
{
  $finder = pakeFinder::type('file')->ignore_version_control();  
  foreach($args as $arg){
    if($arg=='') $arg = '*';else $arg .= '_';
    $finder->name($arg.'schema.yml');
  }
  $finder->prune('doctrine');
  $dirs = array('config');
...
function _propel_convert_xml_schema_by_file($check_schema = true, $prefix = '')
{
...
function _propel_copy_xml_schema_from_plugins_by_file($prefix = '', $args = array())
{
  //$schemas = pakeFinder::type('file')->ignore_version_control()->name($yml.'schema.xml')->prune('doctrine')->in(glob(sfConfig::get('sf_root_dir').'/plugins/*/config'));
  $schemas = array();
  foreach($args as $arg){
    if($arg=='') $arg = '*';else $arg .= '_';
    $t = pakeFinder::type('file')->ignore_version_control()->name('*'.$arg.'schema.xml')->prune('doctrine')->in(glob(sfConfig::get('sf_root_dir').'/plugins/*/config'));
    $schemas = array_merge($schemas, $t);
  }
    
  //$s = pakeFinder::type('file')->ignore_version_control()->name($yml.'schema.xml')->prune('doctrine')->in(glob(sfConfig::get('sf_root_dir').'/config/schemas'));
  $s = array();
  foreach($args as $arg){
    if($arg=='') $arg = '*';else $arg .= '_';
    $t = pakeFinder::type('file')->ignore_version_control()->name('*'.$arg.'schema.xml')->prune('doctrine')->in(glob(sfConfig::get('sf_root_dir').'/config/schemas'));
    $s = array_merge($s, $t);
  }
...
function run_propel_build_model_by_file($task, $args)
{
  _propel_convert_yml_schema_by_file(false, 'generated-',$args);
  _propel_copy_xml_schema_from_plugins_by_file('generated-',$args);
  _call_phing_by_file($task, 'om');
  $finder = pakeFinder::type('file')->ignore_version_control()->name('generated-*schema*.xml');
  pake_remove($finder, array('config', 'plugins'));
}

function run_propel_build_sql_by_file($task, $args)
{
  _propel_convert_yml_schema_by_file(false, 'generated-',$args);
  _propel_copy_xml_schema_from_plugins_by_file('generated-',$args);
...
function _call_phing_by_file($task, $task_name, $check_schema = true)
{
