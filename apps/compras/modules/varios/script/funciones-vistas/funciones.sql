...
SELECT INTO redondeo COALESCE(redondeo,'N')
FROM NPDEFGEN;
...
(CASE WHEN COALESCE(B.ANTAP,''N'')=''N'' THEN antpub(''A'',A.CODEMP,A.FECFIN,''N'')
ELSE antpub(''A'',A.CODEMP,A.FECFIN,''S'') END) AS ANTANNOSNUEVOREG,
(CASE WHEN COALESCE(B.ANTAP,''N'')=''N'' THEN antpub(''M'',A.CODEMP,A.FECFIN,''N'')
ELSE antpub(''M'',A.CODEMP,A.FECFIN,''S'') END) AS ANTMESESNUEVOREG,
(CASE WHEN COALESCE(B.ANTAP,''N'')=''N'' THEN antpub(''D'',A.CODEMP,A.FECFIN,''N'')
ELSE antpub(''D'',A.CODEMP,A.FECFIN,''S'') END) AS ANTDIASNUEVOREG,
NOW()::DATE AS FECINIREG
FROM
(SELECT A.CODEMP,A.NOMEMP,B.CODTIPCON,A.FECING,A.FECING AS FECINI,A.FECCORACU AS FECFIN,A.FECING AS FECINIREAL,A.FECCORACU AS FECFINREAL,
COALESCE(A.CAPACTACU,0) AS MONTO,0 AS MONDIA,0 AS ALIUTI,0 AS ALIVAC,0 AS ALIADI,0 AS MONDIAPRO,COALESCE(A.DIAACU,0)+COALESCE(A.DIAADIACU,0) AS DIAS,
...
(CASE WHEN A.DIAS<>5 THEN
   (CASE WHEN COALESCE(B.CALESPPRES,''N'')=''N'' THEN
      (SELECT ROUND(AVG(MONDIA/(CASE WHEN FECINI<TO_DATE(''01/01/2008'',''DD/MM/YYYY'') THEN 1000 ELSE 1 END)),2) FROM NPPRESTACIONES
       WHERE CODEMP='''||codigo||'''
       AND ID<=A.ID
       AND ID>=A.ID-11)
    ELSE
      (SELECT ROUND(AVG((SALNOR+BONOUTI+BONOVAC+BONOADI)/(CASE WHEN FECINI<TO_DATE(''01/01/2008'',''DD/MM/YYYY'') THEN 1000 ELSE 1 END))/30,2) FROM NPPRESTACIONES
       WHERE CODEMP='''||codigo||'''
       AND ID<=A.ID
       AND ID>=A.ID-11)
    END)
...
A.ANNOSNUEVOREGIMEN::NUMERIC AS ANTANNOSNUEVOREG,
A.MESESNUEVOREGIMEN::NUMERIC AS ANTMESESNUEVOREG,A.DIASNUEVOREGIMEN::NUMERIC AS ANTDIASNUEVOREG,
A.FECINIREG
FROM NPPRESTACIONES A,NPDEFGEN B
where A.CODEMP='''||codigo||'''
AND A.FECFIN>=(CASE WHEN TO_CHAR(A.FECING,''DD'')>''01'' THEN ADD_MONTHS(A.FECING,4)
...
(CASE WHEN COALESCE(B.TIPSALAJUNODEP,'' '')=''SN'' THEN A.SALNOR
       ...
       WHEN COALESCE(B.TIPSALAJUNODEP,'' '')=''SP'' THEN (SELECT ROUND(AVG(MONDIA/(CASE WHEN FECINI<TO_DATE(''01/01/2008'',''DD/MM/YYYY'') THEN 1000 ELSE 1 END)),2)*30 FROM NPPRESTACIONES
					WHERE CODEMP='''||codigo||'''
...
(CASE WHEN COALESCE(B.TIPSALAJUNODEP,'' '')=''SN'' THEN ROUND(A.SALNOR/30,2)
       ...
       WHEN COALESCE(B.TIPSALAJUNODEP,'' '')=''SP'' THEN (SELECT ROUND(AVG(MONDIA/(CASE WHEN FECINI<TO_DATE(''01/01/2008'',''DD/MM/YYYY'') THEN 1000 ELSE 1 END)),2) FROM NPPRESTACIONES
					WHERE CODEMP='''||codigo||'''
					AND FECINI > (CASE WHEN TO_DATE(TO_CHAR(FECING,''DD/MM/'')||
									SUBSTR('''||fechareal||''',7),''DD/MM/YYYY'')<=
								TO_DATE('''||fechareal||''',''DD/MM/YYYY'')
	...
	ELSE (CASE WHEN A.ALICUOCON<>0 THEN A.MONDIA
	      ELSE ROUND(A.SALNOR/30,2) END) END)::NUMERIC AS MONDIA,A.ALIUTI::NUMERIC ,A.ALIVAC::NUMERIC ,A.ALIADI::NUMERIC ,0::NUMERIC AS MONDIAPRO,
COALESCE((CASE WHEN A.ANNOANTIG>0 THEN
 ...
 ELSE (CASE WHEN (A.MESANTIGTOT=6 AND A.DIAANTIGTOT>0) OR A.MESANTIGTOT>6
       ...
       THEN 15-(SELECT COALESCE(SUM(5),0)
                FROM NPPRESTACIONES
                WHERE CODEMP='''||codigo||'''
                AND FECFIN>=(CASE WHEN TO_CHAR(FECING,''DD'')>''01'' THEN ADD_MONTHS(FECING,4)
                               ELSE ADD_MONTHS(FECING,4)-1 END)
                AND FECFIN <= TO_DATE('''||fechareal||''',''DD/MM/YYYY'')) END) END),0)::INTEGER AS DIAS,
...
A.MONTO::NUMERIC,A.MONDIA::NUMERIC,A.ALIUTI::NUMERIC,A.ALIVAC::NUMERIC,A.ALIADI::NUMERIC,
(CASE WHEN COALESCE(B.CALESPPRES,''N'')=''N'' THEN
	COALESCE((SELECT ROUND(AVG(MONDIA/(CASE WHEN FECINI<TO_DATE(''01/01/2008'',''DD/MM/YYYY'') THEN 1000 ELSE 1 END)),2) FROM NPPRESTACIONES
		  WHERE CODEMP='''||codigo||'''
		  AND FECINI > (CASE WHEN TO_DATE(TO_CHAR(FECING,''DD/MM/'')||
		                          SUBSTR('''||fechareal||''',7),''DD/MM/YYYY'')<=
		                          TO_DATE('''||fechareal||''',''DD/MM/YYYY'')
 ...
 ELSE
       COALESCE((SELECT ROUND(AVG((SALNOR+BONOUTI+BONOVAC+BONOADI)/(CASE WHEN FECINI<TO_DATE(''01/01/2008'',''DD/MM/YYYY'') THEN 1000 ELSE 1 END))/30,2) FROM NPPRESTACIONES
		  WHERE CODEMP='''||codigo||'''
		  AND FECINI > (CASE WHEN TO_DATE(TO_CHAR(FECING,''DD/MM/'')||
		                          SUBSTR('''||fechareal||''',7),''DD/MM/YYYY'')<=
		                          TO_DATE('''||fechareal||''',''DD/MM/YYYY'')
		  ...
		  AND FECFIN <= TO_DATE('''||fechareal||''',''DD/MM/YYYY'')),0)
 END)::NUMERIC AS MONDIAPRO,
(CASE WHEN A.FECING>A.FECINIREG THEN
      (CASE WHEN (A.ANNOANTIGTOT>=1 AND A.MESANTIGTOT=6 AND A.DIAANTIGTOT>0) OR
		(A.ANNOANTIGTOT>=1 AND A.MESANTIGTOT>6) THEN
	  (CASE WHEN (A.ANNOANTIGTOT*2) < 30
           THEN (A.ANNOANTIGTOT*2)
           ELSE 30 END)
      ELSE
	  0
      END)
 ELSE
      (CASE WHEN (A.ANNOSNUEVOREGIMEN>=1 AND A.MESESNUEVOREGIMEN=6 AND A.DIASNUEVOREGIMEN>0) OR
		(A.ANNOSNUEVOREGIMEN>=1 AND A.MESESNUEVOREGIMEN>6) THEN
	  (CASE WHEN ((A.ANNOSNUEVOREGIMEN+1)*2) < 30
           THEN  (A.ANNOSNUEVOREGIMEN+1)*2
           ELSE 30 END)
      ELSE
	  0
      END)
...
SELECT A.CODEMP::VARCHAR,A.NOMEMP::VARCHAR,
A.CODTIPCON::VARCHAR,A.FECING::DATE,A.FECINI::DATE,A.FECFIN::DATE,A.FECINIREAL::DATE,A.FECFINREAL::DATE,
A.MONTO::NUMERIC,0::NUMERIC AS MONDIAPRO,A.ALIUTI::NUMERIC,A.ALIVAC::NUMERIC,A.ALIADI::NUMERIC,A.MONDIA::NUMERIC,
...
loop
  ...
  IF redondeo='S' AND recordSalida.fecini=TO_DATE('01/01/2008','DD/MM/YYYY') THEN
     interesacum:=ROUND(interesacum/1000,2);
     capital:=ROUND(capital/1000,2);
     capitalint:=ROUND(capitalint/1000,2);
     capitalactint:=ROUND(capitalactint/1000,2);
     recordSalida.capital:=ROUND(recordSalida.capital/1000,2);
     recordSalida.monint:=ROUND(recordSalida.monint/1000,2);
  END IF;
...
CREATE OR REPLACE FUNCTION calculopresant(codigo character varying, fecha character varying, capitalizacion character, dias numeric)
  RETURNS SETOF regprestacionesant AS
$BODY$
DECLARE
...
(SELECT COALESCE(SUM(MONANT),0) FROM NPANTPRE
 WHERE CODEMP='''||codigo||'''
 AND TO_CHAR(FECANT,''MM/YYYY'')=TO_CHAR(A.FECFIN,''MM/YYYY'') AND FECANT<=A.FECEGR)::NUMERIC AS MONANT,
...
A.ID::INTEGER,0::NUMERIC AS CAPITAL,
0::NUMERIC AS CAPITALACT,A.TASINT AS TASA,0::NUMERIC AS MONINT,0::NUMERIC AS INTACU,
0::NUMERIC AS MONADEINT,0::NUMERIC AS TOTINTERES,
(CASE WHEN COALESCE(A.FECEGR,NOW()::DATE)>ADD_MONTHS(A.ANNOFIN,-12) THEN A.ANNOANTIG::NUMERIC-1
 ELSE A.ANNOANTIG END) AS ANTANNOS,
(CASE WHEN COALESCE(A.FECEGR,NOW()::DATE)>ADD_MONTHS(A.ANNOFIN,-12) THEN 11::NUMERIC
 ELSE A.MESANTIG END) AS ANTMESES,
(CASE WHEN COALESCE(A.FECEGR,NOW()::DATE)>ADD_MONTHS(A.ANNOFIN,-12) THEN 30::NUMERIC
 ELSE A.DIAANTIG END) AS ANTDIAS,
...
loop
  adelantoint:=0;
  adelantoacu:=adelantoacu+COALESCE(recordSalida.monant,0);
  ...
  ELSE
     IF capitalizacion='M' AND recordSalida.fecfin<>recordSalida.annofin THEN
        ...
        interesacum:=0;
     ELSE
        capital:=COALESCE(recordSalida.monpres,0);
     END IF;
  END IF;
  recordSalida.capital:=capital;

  recordSalida.monint:=(recordSalida.capital* (recordSalida.tasa/100) / diasanno * recordsalida.dias);
  interesacum:=interesacum+COALESCE(recordSalida.monint,0)-COALESCE(recordSalida.monadeint,0);
...
CREATE TYPE regprestaciones AS (
  ...
  "antmeses" NUMERIC,
  "antdias" NUMERIC,
  "antannosnuevoreg" NUMERIC,
  "antmesesnuevoreg" NUMERIC,
  "antdiasnuevoreg" NUMERIC,
  "fecinireg" DATE
);
