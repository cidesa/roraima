    ...
    if ($(id).value!="" && $(id).value!='undefined')
      {
        if (cod1!=cod2){
              ...
              new Ajax.Request(getUrlModuloAjax(), {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=4&codigo='+cod1})
               ...
               alert_('T�tulo presupuestario repetido');}
     }else{
               alert_('El t�tulo presupuestario no puede estar vacio');}

   }else{
               alert_('El t�tulo presupuestario origen no puede estar vacio');}
 ...
 function ajaxexiste(e,id){
  ...
  function escodigoorigen(e,id){
    ...
    while (i<am)
    {
       ...
       if (cod2==codaux){
          alert_('El t�tulo presupuestario no puede ser origen del movimiento');
          $(id).value='';
       }
       }
       i++;
  ...
  function escodigodestino(e,id){
    ...
    while (i<am)
    {
       ...
       var codaux=$(caux).value;
       if (cod1==codaux){
        alert_('El t�tulo presupuestario no puede ser destino del movimiento');
        $(id).value='';
       }
       i++;
    }
