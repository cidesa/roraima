...
abstract class BaseCcanaest extends BaseObject  implements Persistent {
  ...
  public function getCcanalisId()
  {
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = CcanaestPeer::ID;
      }
    ...
    if ($this->ccestado_id !== $v) {
        $this->ccestado_id = $v;
        $this->modifiedColumns[] = CcanaestPeer::CCESTADO_ID;
      }
    ...
    if ($this->ccanalis_id !== $v) {
        $this->ccanalis_id = $v;
        $this->modifiedColumns[] = CcanaestPeer::CCANALIS_ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      throw new PropelException("Error populating Ccanaest object", $e);
    }
  }
  ...
  public function __call($m, $a)
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			CcanaestPeer::doDelete($this, $con);
		...
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aCcanalis !== null) {
				...
				$this->setCcanalis($this->aCcanalis);
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CcanaestPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += CcanaestPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = CcanaestPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = CcanaestPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = CcanaestPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CcanaestPeer::ID)) $criteria->add(CcanaestPeer::ID, $this->id);
		if ($this->isColumnModified(CcanaestPeer::CCESTADO_ID)) $criteria->add(CcanaestPeer::CCESTADO_ID, $this->ccestado_id);
		if ($this->isColumnModified(CcanaestPeer::CCANALIS_ID)) $criteria->add(CcanaestPeer::CCANALIS_ID, $this->ccanalis_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CcanaestPeer::ID, $this->id);
