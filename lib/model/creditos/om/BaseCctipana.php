...
abstract class BaseCctipana extends BaseObject  implements Persistent {
  ...
  public function getNomtipana()
  {

    return trim($this->nomtipana);

  }
  
  public function getDestipana()
  {

    return trim($this->destipana);
  ...
  public function getCcgerencId()
  {
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = CctipanaPeer::ID;
      }
  
	} 
	
	public function setNomtipana($v)
	{

    if ($this->nomtipana !== $v) {
        $this->nomtipana = $v;
        $this->modifiedColumns[] = CctipanaPeer::NOMTIPANA;
      }
  
	} 
	
	public function setDestipana($v)
	{

    if ($this->destipana !== $v) {
        $this->destipana = $v;
        $this->modifiedColumns[] = CctipanaPeer::DESTIPANA;
      }
    ...
    if ($this->ccgerenc_id !== $v) {
        $this->ccgerenc_id = $v;
        $this->modifiedColumns[] = CctipanaPeer::CCGERENC_ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->nomtipana = $rs->getString($startcol + 1);

      $this->destipana = $rs->getString($startcol + 2);

      $this->ccgerenc_id = $rs->getInt($startcol + 3);
      ...
      throw new PropelException("Error populating Cctipana object", $e);
    }
  }
  ...
  public function __call($m, $a)
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			CctipanaPeer::doDelete($this, $con);
		...
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aCcgerenc !== null) {
				...
				$this->setCcgerenc($this->aCcgerenc);
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CctipanaPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += CctipanaPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = CctipanaPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = CctipanaPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = CctipanaPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getNomtipana(),
			$keys[2] => $this->getDestipana(),
			$keys[3] => $this->getCcgerencId(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				$this->setNomtipana($value);
				break;
			case 2:
				$this->setDestipana($value);
				break;
			case 3:
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setNomtipana($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setDestipana($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setCcgerencId($arr[$keys[3]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CctipanaPeer::ID)) $criteria->add(CctipanaPeer::ID, $this->id);
		if ($this->isColumnModified(CctipanaPeer::NOMTIPANA)) $criteria->add(CctipanaPeer::NOMTIPANA, $this->nomtipana);
		if ($this->isColumnModified(CctipanaPeer::DESTIPANA)) $criteria->add(CctipanaPeer::DESTIPANA, $this->destipana);
		if ($this->isColumnModified(CctipanaPeer::CCGERENC_ID)) $criteria->add(CctipanaPeer::CCGERENC_ID, $this->ccgerenc_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CctipanaPeer::ID, $this->id);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setNomtipana($this->nomtipana);

		$copyObj->setDestipana($this->destipana);
		...
		if ($this->collCcanaliss === null) {
			if ($this->isNew()) {
			   $this->collCcanaliss = array();
				...
				$criteria->add(CcanalisPeer::CCTIPANA_ID, $this->getId());
