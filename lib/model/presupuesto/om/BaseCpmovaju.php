...
abstract class BaseCpmovaju extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->refaju !== $v) {
		...
		if ($this->aCpajuste !== null && $this->aCpajuste->getRefaju() !== $v) {
			$this->aCpajuste = null;
		}
  ...
  public function __call($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aCpajuste !== null) {
				if ($this->aCpajuste->isModified()) {
					$affectedRows += $this->aCpajuste->save($con);
				}
				$this->setCpajuste($this->aCpajuste);
			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aCpajuste !== null) {
				if (!$this->aCpajuste->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aCpajuste->getValidationFailures());
				}
			}
	...
	public function getCpajuste($con = null)
	{
		if ($this->aCpajuste === null && (($this->refaju !== "" && $this->refaju !== null))) {
						include_once 'lib/model/presupuesto/om/BaseCpajustePeer.php';

      $c = new Criteria();
      $c->add(CpajustePeer::REFAJU,$this->refaju);
      
			$this->aCpajuste = CpajustePeer::doSelectOne($c, $con);

			
		}
		return $this->aCpajuste;
	}
