...
abstract class BaseCpsoltraslaPeer {
	...
	public static function doCountJoinCptrasla(Criteria $criteria, $distinct = false, $con = null)
	{
		...
		$criteria->addJoin(CpsoltraslaPeer::REFTRA, CptraslaPeer::REFTRA);
	...
	public static function doSelectJoinCptrasla(Criteria $c, $con = null)
	{
		...
		$c->addJoin(CpsoltraslaPeer::REFTRA, CptraslaPeer::REFTRA);
			...
			$obj1->hydrate($rs);
				...
				for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
					...
					$temp_obj3 = $temp_obj1->getCpartley(); 					if ($temp_obj3->getPrimaryKey() === $obj3->getPrimaryKey()) {
						$newObject = false;
						$temp_obj3->addCpsoltrasla($obj1); 						break;
					}
				}

				if ($newObject) {
					$obj3->initCpsoltraslas();
					$obj3->addCpsoltrasla($obj1);
				}
