...
abstract class BaseCpimppag extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->refpag !== $v) {
		...
		if ($this->aCppagos !== null && $this->aCppagos->getRefpag() !== $v) {
			$this->aCppagos = null;
		}
  ...
  public function __call($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aCppagos !== null) {
				if ($this->aCppagos->isModified()) {
					$affectedRows += $this->aCppagos->save($con);
				}
				$this->setCppagos($this->aCppagos);
			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aCppagos !== null) {
				if (!$this->aCppagos->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aCppagos->getValidationFailures());
				}
			}
	...
	public function getCppagos($con = null)
	{
		if ($this->aCppagos === null && (($this->refpag !== "" && $this->refpag !== null))) {
						include_once 'lib/model/presupuesto/om/BaseCppagosPeer.php';

      $c = new Criteria();
      $c->add(CppagosPeer::REFPAG,$this->refpag);
      
			$this->aCppagos = CppagosPeer::doSelectOne($c, $con);

			
		}
		return $this->aCppagos;
	}
