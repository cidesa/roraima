...
class FcsolnegMapBuilder {
	...
	const CLASS_NAME = 'lib.model.hacienda.map.FcsolnegMapBuilder';
	...
	public function doBuild()
	{
		...
		$tMap->setPrimaryKeyMethodInfo('fcsolneg_SEQ');

		$tMap->addColumn('NUMNEG', 'Numneg', 'string', CreoleTypes::VARCHAR, true, 10);

		$tMap->addColumn('NUMSOL', 'Numsol', 'string', CreoleTypes::VARCHAR, true, 12);

		$tMap->addColumn('MOTNEG', 'Motneg', 'string', CreoleTypes::VARCHAR, false, 210);

		$tMap->addColumn('FECNEG', 'Fecneg', 'int', CreoleTypes::DATE, true, null);
