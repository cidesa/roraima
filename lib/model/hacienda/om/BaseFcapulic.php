...
abstract class BaseFcapulic extends BaseObject  implements Persistent {
	...
	protected $aFcconrepRelatedByRifcon;

	
	protected $aFcconrepRelatedByRifrep;
  ...
  public function getHoraapu($format = 'H:i:s')
  {

    if ($this->horaapu === null || $this->horaapu === '') {
      return null;
    } elseif (!is_int($this->horaapu)) {
            $ts = strtotime($this->horaapu);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [horaapu] as date/time value: " . var_export($this->horaapu, true));
      }
    } else {
      $ts = $this->horaapu;
    }
  ...
  public function getId()
  {
    ...
    if ($this->rifcon !== $v) {
		...
		if ($this->aFcconrepRelatedByRifcon !== null && $this->aFcconrepRelatedByRifcon->getRifcon() !== $v) {
			$this->aFcconrepRelatedByRifcon = null;
		}
    ...
    if ($this->rifrep !== $v) {
		...
		if ($this->aFcconrepRelatedByRifrep !== null && $this->aFcconrepRelatedByRifrep->getRifcon() !== $v) {
			$this->aFcconrepRelatedByRifrep = null;
		}
    ...
    if ($v !== null && !is_int($v)) {
      $ts = strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [horaapu] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
    if ($this->horaapu !== $ts) {
      $this->horaapu = $ts;
      $this->modifiedColumns[] = FcapulicPeer::HORAAPU;
    }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->horaapu = $rs->getTime($startcol + 21, null);
  ...
  public function __call($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aFcconrepRelatedByRifcon !== null) {
				if ($this->aFcconrepRelatedByRifcon->isModified()) {
					$affectedRows += $this->aFcconrepRelatedByRifcon->save($con);
				}
				$this->setFcconrepRelatedByRifcon($this->aFcconrepRelatedByRifcon);
			}

			if ($this->aFcconrepRelatedByRifrep !== null) {
				if ($this->aFcconrepRelatedByRifrep->isModified()) {
					$affectedRows += $this->aFcconrepRelatedByRifrep->save($con);
				}
				$this->setFcconrepRelatedByRifrep($this->aFcconrepRelatedByRifrep);
			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aFcconrepRelatedByRifcon !== null) {
				if (!$this->aFcconrepRelatedByRifcon->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aFcconrepRelatedByRifcon->getValidationFailures());
				}
			}

			if ($this->aFcconrepRelatedByRifrep !== null) {
				if (!$this->aFcconrepRelatedByRifrep->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aFcconrepRelatedByRifrep->getValidationFailures());
				}
			}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[21], $arr)) $this->setHoraapu($arr[$keys[21]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FcapulicPeer::HORAAPU)) $criteria->add(FcapulicPeer::HORAAPU, $this->horaapu);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setHoraapu($this->horaapu);
	...
	public function setFcconrepRelatedByRifcon($v)
	{
		...
		$this->aFcconrepRelatedByRifcon = $v;
	}


	
	public function getFcconrepRelatedByRifcon($con = null)
	{
		if ($this->aFcconrepRelatedByRifcon === null && (($this->rifcon !== "" && $this->rifcon !== null))) {
						include_once 'lib/model/hacienda/om/BaseFcconrepPeer.php';

      $c = new Criteria();
      $c->add(FcconrepPeer::RIFCON,$this->rifcon);
      
			$this->aFcconrepRelatedByRifcon = FcconrepPeer::doSelectOne($c, $con);

			
		}
		return $this->aFcconrepRelatedByRifcon;
	}

	
	public function setFcconrepRelatedByRifrep($v)
	{
		...
		if ($v === null) {
			...
			$this->setRifrep($v->getRifcon());
		}


		$this->aFcconrepRelatedByRifrep = $v;
	}


	
	public function getFcconrepRelatedByRifrep($con = null)
	{
		if ($this->aFcconrepRelatedByRifrep === null && (($this->rifrep !== "" && $this->rifrep !== null))) {
						include_once 'lib/model/hacienda/om/BaseFcconrepPeer.php';

      $c = new Criteria();
      $c->add(FcconrepPeer::RIFCON,$this->rifrep);
      
			$this->aFcconrepRelatedByRifrep = FcconrepPeer::doSelectOne($c, $con);

			
		}
		return $this->aFcconrepRelatedByRifrep;
	}
