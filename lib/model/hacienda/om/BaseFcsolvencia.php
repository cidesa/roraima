...
abstract class BaseFcsolvencia extends BaseObject  implements Persistent {
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->funrec = $rs->getString($startcol + 12);

      $this->fecrec = $rs->getDate($startcol + 13, null);
  ...
  public function __call($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[12] => $this->getFunrec(),
			$keys[13] => $this->getFecrec(),
			$keys[14] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[12], $arr)) $this->setFunrec($arr[$keys[12]]);
		if (array_key_exists($keys[13], $arr)) $this->setFecrec($arr[$keys[13]]);
