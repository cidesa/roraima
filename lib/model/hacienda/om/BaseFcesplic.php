...
abstract class BaseFcesplic extends BaseObject  implements Persistent {
  ...
  public function getHorespi($format = 'H:i:s')
  {

    if ($this->horespi === null || $this->horespi === '') {
      return null;
    } elseif (!is_int($this->horespi)) {
            $ts = strtotime($this->horespi);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [horespi] as date/time value: " . var_export($this->horespi, true));
      }
    } else {
      $ts = $this->horespi;
    }
  ...
  public function getHorespf($format = 'H:i:s')
  {

    if ($this->horespf === null || $this->horespf === '') {
      return null;
    } elseif (!is_int($this->horespf)) {
            $ts = strtotime($this->horespf);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [horespf] as date/time value: " . var_export($this->horespf, true));
      }
    } else {
      $ts = $this->horespf;
    }
  ...
  public function getId()
  {
    ...
    if ($v !== null && !is_int($v)) {
      $ts = strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [horespi] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
    ...
    if ($v !== null && !is_int($v)) {
      $ts = strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [horespf] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->horespi = $rs->getTime($startcol + 18, null);

      $this->horespf = $rs->getTime($startcol + 19, null);

      $this->nroent = $rs->getFloat($startcol + 20);

      $this->monent = $rs->getFloat($startcol + 21);

      $this->exoesp = $rs->getString($startcol + 22);
  ...
  public function __call($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[19] => $this->getHorespf(),
			$keys[20] => $this->getNroent(),
			$keys[21] => $this->getMonent(),
			$keys[22] => $this->getExoesp(),
			$keys[23] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[19], $arr)) $this->setHorespf($arr[$keys[19]]);
		if (array_key_exists($keys[20], $arr)) $this->setNroent($arr[$keys[20]]);
		if (array_key_exists($keys[21], $arr)) $this->setMonent($arr[$keys[21]]);
		if (array_key_exists($keys[22], $arr)) $this->setExoesp($arr[$keys[22]]);
