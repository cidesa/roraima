...
abstract class BaseFaartfac extends BaseObject  implements Persistent {
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->cantot = $rs->getFloat($startcol + 3);

      $this->precio = $rs->getFloat($startcol + 4);

      $this->monrgo = $rs->getFloat($startcol + 5);

      $this->mondes = $rs->getFloat($startcol + 6);

      $this->totart = $rs->getFloat($startcol + 7);

      $this->canaju = $rs->getFloat($startcol + 8);
  ...
  public function __call($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[3] => $this->getCantot(),
			$keys[4] => $this->getPrecio(),
			$keys[5] => $this->getMonrgo(),
			$keys[6] => $this->getMondes(),
			$keys[7] => $this->getTotart(),
			$keys[8] => $this->getCanaju(),
			$keys[9] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setCantot($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setPrecio($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setMonrgo($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setMondes($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setTotart($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setCanaju($arr[$keys[8]]);
