...
abstract class BaseNppartidasPeer {
	...
	const CLASS_DEFAULT = 'lib.model.Nppartidas';
	...
	{
		include_once 'lib/model/map/NppartidasMapBuilder.php';
		return BasePeer::getMapBuilder('lib.model.map.NppartidasMapBuilder');
	}
	...
	public static function doUpdate($values, $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$comparison = $criteria->getComparison(NppartidasPeer::ID);
			$selectCriteria->add(NppartidasPeer::ID, $criteria->remove(NppartidasPeer::ID), $comparison);
	...
	public static function doDeleteAll($con = null)
	{
		...
		$affectedRows = 0; 		try {
									$con->begin();
			$affectedRows += BasePeer::doDeleteAll(NppartidasPeer::TABLE_NAME, $con);
			$con->commit();
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollback();
	 ...
	 public static function doDelete($values, $con = null)
	 {
		...
		if ($values instanceof Criteria) {
			$criteria = clone $values; 		} elseif ($values instanceof Nppartidas) {

			$criteria = $values->buildPkeyCriteria();
		} else {
						$criteria = new Criteria(self::DATABASE_NAME);
			$criteria->add(NppartidasPeer::ID, (array) $values, Criteria::IN);
		}
	...
	public static function doValidate(Nppartidas $obj, $cols = null)
	{
		$columns = array();

		if ($cols) {
			$dbMap = Propel::getDatabaseMap(NppartidasPeer::DATABASE_NAME);
			$tableMap = $dbMap->getTable(NppartidasPeer::TABLE_NAME);
		...
		$res =  BasePeer::doValidate(NppartidasPeer::DATABASE_NAME, NppartidasPeer::TABLE_NAME, $columns);
    if ($res !== true) {
        $request = sfContext::getInstance()->getRequest();
        foreach ($res as $failed) {
            $col = NppartidasPeer::translateFieldname($failed->getColumn(), BasePeer::TYPE_COLNAME, BasePeer::TYPE_PHPNAME);
...
if (Propel::isInit()) {
		...
		Propel::log('Could not initialize Peer: ' . $e->getMessage(), Propel::LOG_ERR);
			...
			require_once 'lib/model/map/NppartidasMapBuilder.php';
	Propel::registerMapBuilder('lib.model.map.NppartidasMapBuilder');
}
