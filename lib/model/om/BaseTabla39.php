...
abstract class BaseTabla39 extends BaseObject  implements Persistent {
	...
	public function setRefaju($v)
	{
		...
		if ($this->refaju !== $v) {
			$this->refaju = $v;
			$this->modifiedColumns[] = Tabla39Peer::REFAJU;
		}
	...
	public function setTipaju($v)
	{
		...
		if ($this->tipaju !== $v) {
			$this->tipaju = $v;
			$this->modifiedColumns[] = Tabla39Peer::TIPAJU;
		}
	...
	public function setFecaju($v)
	{
		...
		if ($this->fecaju !== $ts) {
			$this->fecaju = $ts;
			$this->modifiedColumns[] = Tabla39Peer::FECAJU;
		}
	...
	public function setAnoaju($v)
	{
		...
		if ($this->anoaju !== $v) {
			$this->anoaju = $v;
			$this->modifiedColumns[] = Tabla39Peer::ANOAJU;
		}
	...
	public function setRefere($v)
	{
		...
		if ($this->refere !== $v) {
			$this->refere = $v;
			$this->modifiedColumns[] = Tabla39Peer::REFERE;
		}
	...
	public function setDesaju($v)
	{
		...
		if ($this->desaju !== $v) {
			$this->desaju = $v;
			$this->modifiedColumns[] = Tabla39Peer::DESAJU;
		}
	...
	public function setDesanu($v)
	{
		...
		if ($this->desanu !== $v) {
			$this->desanu = $v;
			$this->modifiedColumns[] = Tabla39Peer::DESANU;
		}
	...
	public function setTotaju($v)
	{
		...
		if ($this->totaju !== $v) {
			$this->totaju = $v;
			$this->modifiedColumns[] = Tabla39Peer::TOTAJU;
		}
	...
	public function setStaaju($v)
	{
		...
		if ($this->staaju !== $v) {
			$this->staaju = $v;
			$this->modifiedColumns[] = Tabla39Peer::STAAJU;
		}
	...
	public function setFecanu($v)
	{
		...
		if ($this->fecanu !== $ts) {
			$this->fecanu = $ts;
			$this->modifiedColumns[] = Tabla39Peer::FECANU;
		}
	...
	public function setNumcom($v)
	{
		...
		if ($this->numcom !== $v) {
			$this->numcom = $v;
			$this->modifiedColumns[] = Tabla39Peer::NUMCOM;
		}
	...
	public function setCuoanu($v)
	{
		...
		if ($this->cuoanu !== $v) {
			$this->cuoanu = $v;
			$this->modifiedColumns[] = Tabla39Peer::CUOANU;
		}
	...
	public function setFecanudes($v)
	{
		...
		if ($this->fecanudes !== $ts) {
			$this->fecanudes = $ts;
			$this->modifiedColumns[] = Tabla39Peer::FECANUDES;
		}
	...
	public function setFecanuhas($v)
	{
		...
		if ($this->fecanuhas !== $ts) {
			$this->fecanuhas = $ts;
			$this->modifiedColumns[] = Tabla39Peer::FECANUHAS;
		}
	...
	public function setOrdpag($v)
	{
		...
		if ($this->ordpag !== $v) {
			$this->ordpag = $v;
			$this->modifiedColumns[] = Tabla39Peer::ORDPAG;
		}
	...
	public function setFecenvcon($v)
	{
		...
		if ($this->fecenvcon !== $ts) {
			$this->fecenvcon = $ts;
			$this->modifiedColumns[] = Tabla39Peer::FECENVCON;
		}
	...
	public function setFecenvfin($v)
	{
		...
		if ($this->fecenvfin !== $ts) {
			$this->fecenvfin = $ts;
			$this->modifiedColumns[] = Tabla39Peer::FECENVFIN;
		}
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = Tabla39Peer::ID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			throw new PropelException("Error populating Tabla39 object", $e);
		}
	}
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = Tabla39Peer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setNew(false);
				} else {
					$affectedRows += Tabla39Peer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Tabla39Peer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Tabla39Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = Tabla39Peer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Tabla39Peer::REFAJU)) $criteria->add(Tabla39Peer::REFAJU, $this->refaju);
		if ($this->isColumnModified(Tabla39Peer::TIPAJU)) $criteria->add(Tabla39Peer::TIPAJU, $this->tipaju);
		if ($this->isColumnModified(Tabla39Peer::FECAJU)) $criteria->add(Tabla39Peer::FECAJU, $this->fecaju);
		if ($this->isColumnModified(Tabla39Peer::ANOAJU)) $criteria->add(Tabla39Peer::ANOAJU, $this->anoaju);
		if ($this->isColumnModified(Tabla39Peer::REFERE)) $criteria->add(Tabla39Peer::REFERE, $this->refere);
		if ($this->isColumnModified(Tabla39Peer::DESAJU)) $criteria->add(Tabla39Peer::DESAJU, $this->desaju);
		if ($this->isColumnModified(Tabla39Peer::DESANU)) $criteria->add(Tabla39Peer::DESANU, $this->desanu);
		if ($this->isColumnModified(Tabla39Peer::TOTAJU)) $criteria->add(Tabla39Peer::TOTAJU, $this->totaju);
		if ($this->isColumnModified(Tabla39Peer::STAAJU)) $criteria->add(Tabla39Peer::STAAJU, $this->staaju);
		if ($this->isColumnModified(Tabla39Peer::FECANU)) $criteria->add(Tabla39Peer::FECANU, $this->fecanu);
		if ($this->isColumnModified(Tabla39Peer::NUMCOM)) $criteria->add(Tabla39Peer::NUMCOM, $this->numcom);
		if ($this->isColumnModified(Tabla39Peer::CUOANU)) $criteria->add(Tabla39Peer::CUOANU, $this->cuoanu);
		if ($this->isColumnModified(Tabla39Peer::FECANUDES)) $criteria->add(Tabla39Peer::FECANUDES, $this->fecanudes);
		if ($this->isColumnModified(Tabla39Peer::FECANUHAS)) $criteria->add(Tabla39Peer::FECANUHAS, $this->fecanuhas);
		if ($this->isColumnModified(Tabla39Peer::ORDPAG)) $criteria->add(Tabla39Peer::ORDPAG, $this->ordpag);
		if ($this->isColumnModified(Tabla39Peer::FECENVCON)) $criteria->add(Tabla39Peer::FECENVCON, $this->fecenvcon);
		if ($this->isColumnModified(Tabla39Peer::FECENVFIN)) $criteria->add(Tabla39Peer::FECENVFIN, $this->fecenvfin);
		if ($this->isColumnModified(Tabla39Peer::ID)) $criteria->add(Tabla39Peer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(Tabla39Peer::ID, $this->id);
