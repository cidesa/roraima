...
abstract class BaseTabla5 extends BaseObject  implements Persistent {
	...
	public function setRefpag($v)
	{
		...
		if ($this->refpag !== $v) {
			$this->refpag = $v;
			$this->modifiedColumns[] = Tabla5Peer::REFPAG;
		}
	...
	public function setCodpre($v)
	{
		...
		if ($this->codpre !== $v) {
			$this->codpre = $v;
			$this->modifiedColumns[] = Tabla5Peer::CODPRE;
		}
	...
	public function setMonimp($v)
	{
		...
		if ($this->monimp !== $v) {
			$this->monimp = $v;
			$this->modifiedColumns[] = Tabla5Peer::MONIMP;
		}
	...
	public function setMonaju($v)
	{
		...
		if ($this->monaju !== $v) {
			$this->monaju = $v;
			$this->modifiedColumns[] = Tabla5Peer::MONAJU;
		}
	...
	public function setStaimp($v)
	{
		...
		if ($this->staimp !== $v) {
			$this->staimp = $v;
			$this->modifiedColumns[] = Tabla5Peer::STAIMP;
		}
	...
	public function setRefere($v)
	{
		...
		if ($this->refere !== $v) {
			$this->refere = $v;
			$this->modifiedColumns[] = Tabla5Peer::REFERE;
		}
	...
	public function setRefprc($v)
	{
		...
		if ($this->refprc !== $v) {
			$this->refprc = $v;
			$this->modifiedColumns[] = Tabla5Peer::REFPRC;
		}
	...
	public function setRefcom($v)
	{
		...
		if ($this->refcom !== $v) {
			$this->refcom = $v;
			$this->modifiedColumns[] = Tabla5Peer::REFCOM;
		}
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = Tabla5Peer::ID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			throw new PropelException("Error populating Tabla5 object", $e);
		}
	}
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = Tabla5Peer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setNew(false);
				} else {
					$affectedRows += Tabla5Peer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Tabla5Peer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Tabla5Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = Tabla5Peer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Tabla5Peer::REFPAG)) $criteria->add(Tabla5Peer::REFPAG, $this->refpag);
		if ($this->isColumnModified(Tabla5Peer::CODPRE)) $criteria->add(Tabla5Peer::CODPRE, $this->codpre);
		if ($this->isColumnModified(Tabla5Peer::MONIMP)) $criteria->add(Tabla5Peer::MONIMP, $this->monimp);
		if ($this->isColumnModified(Tabla5Peer::MONAJU)) $criteria->add(Tabla5Peer::MONAJU, $this->monaju);
		if ($this->isColumnModified(Tabla5Peer::STAIMP)) $criteria->add(Tabla5Peer::STAIMP, $this->staimp);
		if ($this->isColumnModified(Tabla5Peer::REFERE)) $criteria->add(Tabla5Peer::REFERE, $this->refere);
		if ($this->isColumnModified(Tabla5Peer::REFPRC)) $criteria->add(Tabla5Peer::REFPRC, $this->refprc);
		if ($this->isColumnModified(Tabla5Peer::REFCOM)) $criteria->add(Tabla5Peer::REFCOM, $this->refcom);
		if ($this->isColumnModified(Tabla5Peer::ID)) $criteria->add(Tabla5Peer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(Tabla5Peer::ID, $this->id);
