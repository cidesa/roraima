...
abstract class BaseTabla19 extends BaseObject  implements Persistent {
	...
	public function setRefcau($v)
	{
		...
		if ($this->refcau !== $v) {
			$this->refcau = $v;
			$this->modifiedColumns[] = Tabla19Peer::REFCAU;
		}
	...
	public function setCodpre($v)
	{
		...
		if ($this->codpre !== $v) {
			$this->codpre = $v;
			$this->modifiedColumns[] = Tabla19Peer::CODPRE;
		}
	...
	public function setMonimp($v)
	{
		...
		if ($this->monimp !== $v) {
			$this->monimp = $v;
			$this->modifiedColumns[] = Tabla19Peer::MONIMP;
		}
	...
	public function setMonpag($v)
	{
		...
		if ($this->monpag !== $v) {
			$this->monpag = $v;
			$this->modifiedColumns[] = Tabla19Peer::MONPAG;
		}
	...
	public function setMonaju($v)
	{
		...
		if ($this->monaju !== $v) {
			$this->monaju = $v;
			$this->modifiedColumns[] = Tabla19Peer::MONAJU;
		}
	...
	public function setStaimp($v)
	{
		...
		if ($this->staimp !== $v) {
			$this->staimp = $v;
			$this->modifiedColumns[] = Tabla19Peer::STAIMP;
		}
	...
	public function setRefere($v)
	{
		...
		if ($this->refere !== $v) {
			$this->refere = $v;
			$this->modifiedColumns[] = Tabla19Peer::REFERE;
		}
	...
	public function setRefprc($v)
	{
		...
		if ($this->refprc !== $v) {
			$this->refprc = $v;
			$this->modifiedColumns[] = Tabla19Peer::REFPRC;
		}
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = Tabla19Peer::ID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			throw new PropelException("Error populating Tabla19 object", $e);
		}
	}
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = Tabla19Peer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setNew(false);
				} else {
					$affectedRows += Tabla19Peer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Tabla19Peer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Tabla19Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = Tabla19Peer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Tabla19Peer::REFCAU)) $criteria->add(Tabla19Peer::REFCAU, $this->refcau);
		if ($this->isColumnModified(Tabla19Peer::CODPRE)) $criteria->add(Tabla19Peer::CODPRE, $this->codpre);
		if ($this->isColumnModified(Tabla19Peer::MONIMP)) $criteria->add(Tabla19Peer::MONIMP, $this->monimp);
		if ($this->isColumnModified(Tabla19Peer::MONPAG)) $criteria->add(Tabla19Peer::MONPAG, $this->monpag);
		if ($this->isColumnModified(Tabla19Peer::MONAJU)) $criteria->add(Tabla19Peer::MONAJU, $this->monaju);
		if ($this->isColumnModified(Tabla19Peer::STAIMP)) $criteria->add(Tabla19Peer::STAIMP, $this->staimp);
		if ($this->isColumnModified(Tabla19Peer::REFERE)) $criteria->add(Tabla19Peer::REFERE, $this->refere);
		if ($this->isColumnModified(Tabla19Peer::REFPRC)) $criteria->add(Tabla19Peer::REFPRC, $this->refprc);
		if ($this->isColumnModified(Tabla19Peer::ID)) $criteria->add(Tabla19Peer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(Tabla19Peer::ID, $this->id);
