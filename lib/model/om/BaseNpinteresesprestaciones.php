...
abstract class BaseNpinteresesprestaciones extends BaseObject  implements Persistent {
  ...
  public function getFecpago($format = 'Y-m-d')
  {
    ...
    if ($this->fecpago === null || $this->fecpago === '') {
      return null;
            ...
            $ts = adodb_strtotime($this->fecpago);
  ...
  public function getFeccalculo($format = 'Y-m-d')
  {
    ...
    if ($this->feccalculo === null || $this->feccalculo === '') {
      return null;
            ...
            $ts = adodb_strtotime($this->feccalculo);
  ...
  public function getAcufondoantiguedad($val=false)
  {

    if($val) return number_format($this->acufondoantiguedad,2,',','.');
    else return $this->acufondoantiguedad;

  }
  
  public function getAcuinteres($val=false)
  {

    if($val) return number_format($this->acuinteres,2,',','.');
    else return $this->acuinteres;
  ...
  public function getId()
  {
    ...
    if ($this->acufondoantiguedad !== $v) {
        $this->acufondoantiguedad = Herramientas::toFloat($v);
    ...
    if ($this->acuinteres !== $v) {
        $this->acuinteres = Herramientas::toFloat($v);
