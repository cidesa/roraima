...
abstract class BaseTabla13 extends BaseObject  implements Persistent {
	...
	public function setRefpag($v)
	{
		...
		if ($this->refpag !== $v) {
			$this->refpag = $v;
			$this->modifiedColumns[] = Tabla13Peer::REFPAG;
		}
	...
	public function setCodpre($v)
	{
		...
		if ($this->codpre !== $v) {
			$this->codpre = $v;
			$this->modifiedColumns[] = Tabla13Peer::CODPRE;
		}
	...
	public function setMonimp($v)
	{
		...
		if ($this->monimp !== $v) {
			$this->monimp = $v;
			$this->modifiedColumns[] = Tabla13Peer::MONIMP;
		}
	...
	public function setMonaju($v)
	{
		...
		if ($this->monaju !== $v) {
			$this->monaju = $v;
			$this->modifiedColumns[] = Tabla13Peer::MONAJU;
		}
	...
	public function setStaimp($v)
	{
		...
		if ($this->staimp !== $v) {
			$this->staimp = $v;
			$this->modifiedColumns[] = Tabla13Peer::STAIMP;
		}
	...
	public function setRefere($v)
	{
		...
		if ($this->refere !== $v) {
			$this->refere = $v;
			$this->modifiedColumns[] = Tabla13Peer::REFERE;
		}
	...
	public function setRefprc($v)
	{
		...
		if ($this->refprc !== $v) {
			$this->refprc = $v;
			$this->modifiedColumns[] = Tabla13Peer::REFPRC;
		}
	...
	public function setRefcom($v)
	{
		...
		if ($this->refcom !== $v) {
			$this->refcom = $v;
			$this->modifiedColumns[] = Tabla13Peer::REFCOM;
		}
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = Tabla13Peer::ID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			throw new PropelException("Error populating Tabla13 object", $e);
		}
	}
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = Tabla13Peer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setNew(false);
				} else {
					$affectedRows += Tabla13Peer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Tabla13Peer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Tabla13Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = Tabla13Peer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Tabla13Peer::REFPAG)) $criteria->add(Tabla13Peer::REFPAG, $this->refpag);
		if ($this->isColumnModified(Tabla13Peer::CODPRE)) $criteria->add(Tabla13Peer::CODPRE, $this->codpre);
		if ($this->isColumnModified(Tabla13Peer::MONIMP)) $criteria->add(Tabla13Peer::MONIMP, $this->monimp);
		if ($this->isColumnModified(Tabla13Peer::MONAJU)) $criteria->add(Tabla13Peer::MONAJU, $this->monaju);
		if ($this->isColumnModified(Tabla13Peer::STAIMP)) $criteria->add(Tabla13Peer::STAIMP, $this->staimp);
		if ($this->isColumnModified(Tabla13Peer::REFERE)) $criteria->add(Tabla13Peer::REFERE, $this->refere);
		if ($this->isColumnModified(Tabla13Peer::REFPRC)) $criteria->add(Tabla13Peer::REFPRC, $this->refprc);
		if ($this->isColumnModified(Tabla13Peer::REFCOM)) $criteria->add(Tabla13Peer::REFCOM, $this->refcom);
		if ($this->isColumnModified(Tabla13Peer::ID)) $criteria->add(Tabla13Peer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(Tabla13Peer::ID, $this->id);
