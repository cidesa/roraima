...
abstract class BaseNpsalint05022007 extends BaseObject  implements Persistent {
	...
	public function setCodcon($v)
	{
		...
		if ($this->codcon !== $v) {
			$this->codcon = $v;
			$this->modifiedColumns[] = Npsalint05022007Peer::CODCON;
		}
	...
	public function setCodemp($v)
	{
		...
		if ($this->codemp !== $v) {
			$this->codemp = $v;
			$this->modifiedColumns[] = Npsalint05022007Peer::CODEMP;
		}
	...
	public function setCodasi($v)
	{
		...
		if ($this->codasi !== $v) {
			$this->codasi = $v;
			$this->modifiedColumns[] = Npsalint05022007Peer::CODASI;
		}
	...
	public function setMonasi($v)
	{
		...
		if ($this->monasi !== $v) {
			$this->monasi = $v;
			$this->modifiedColumns[] = Npsalint05022007Peer::MONASI;
		}
	...
	public function setFecinicon($v)
	{
		...
		if ($this->fecinicon !== $ts) {
			$this->fecinicon = $ts;
			$this->modifiedColumns[] = Npsalint05022007Peer::FECINICON;
		}
	...
	public function setFecfincon($v)
	{
		...
		if ($this->fecfincon !== $ts) {
			$this->fecfincon = $ts;
			$this->modifiedColumns[] = Npsalint05022007Peer::FECFINCON;
		}
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = Npsalint05022007Peer::ID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			throw new PropelException("Error populating Npsalint05022007 object", $e);
		}
	}
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			Npsalint05022007Peer::doDelete($this, $con);
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = Npsalint05022007Peer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setNew(false);
				} else {
					$affectedRows += Npsalint05022007Peer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Npsalint05022007Peer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Npsalint05022007Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = Npsalint05022007Peer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Npsalint05022007Peer::CODCON)) $criteria->add(Npsalint05022007Peer::CODCON, $this->codcon);
		if ($this->isColumnModified(Npsalint05022007Peer::CODEMP)) $criteria->add(Npsalint05022007Peer::CODEMP, $this->codemp);
		if ($this->isColumnModified(Npsalint05022007Peer::CODASI)) $criteria->add(Npsalint05022007Peer::CODASI, $this->codasi);
		if ($this->isColumnModified(Npsalint05022007Peer::MONASI)) $criteria->add(Npsalint05022007Peer::MONASI, $this->monasi);
		if ($this->isColumnModified(Npsalint05022007Peer::FECINICON)) $criteria->add(Npsalint05022007Peer::FECINICON, $this->fecinicon);
		if ($this->isColumnModified(Npsalint05022007Peer::FECFINCON)) $criteria->add(Npsalint05022007Peer::FECFINCON, $this->fecfincon);
		if ($this->isColumnModified(Npsalint05022007Peer::ID)) $criteria->add(Npsalint05022007Peer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(Npsalint05022007Peer::ID, $this->id);
	...
	{
		if (self::$peer === null) {
			self::$peer = new Npsalint05022007Peer();
		}
		return self::$peer;
	}
