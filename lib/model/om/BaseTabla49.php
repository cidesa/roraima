...
abstract class BaseTabla49 extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->numcom !== $v) {
        $this->numcom = $v;
        $this->modifiedColumns[] = Tabla49Peer::NUMCOM;
      }
    ...
    if ($this->feccom !== $ts) {
      $this->feccom = $ts;
      $this->modifiedColumns[] = Tabla49Peer::FECCOM;
    }
    ...
    if ($this->debcre !== $v) {
        $this->debcre = $v;
        $this->modifiedColumns[] = Tabla49Peer::DEBCRE;
      }
    ...
    if ($this->codcta !== $v) {
        $this->codcta = $v;
        $this->modifiedColumns[] = Tabla49Peer::CODCTA;
      }
    ...
    if ($this->numasi !== $v) {
        ...
        $this->modifiedColumns[] = Tabla49Peer::NUMASI;
      }
    ...
    if ($this->refasi !== $v) {
        $this->refasi = $v;
        $this->modifiedColumns[] = Tabla49Peer::REFASI;
      }
    ...
    if ($this->desasi !== $v) {
        $this->desasi = $v;
        $this->modifiedColumns[] = Tabla49Peer::DESASI;
      }
    ...
    if ($this->monasi !== $v) {
        ...
        $this->modifiedColumns[] = Tabla49Peer::MONASI;
      }
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = Tabla49Peer::ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      throw new PropelException("Error populating Tabla49 object", $e);
    }
  }
  ...
  public function __call($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = Tabla49Peer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += Tabla49Peer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Tabla49Peer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Tabla49Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = Tabla49Peer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Tabla49Peer::NUMCOM)) $criteria->add(Tabla49Peer::NUMCOM, $this->numcom);
		if ($this->isColumnModified(Tabla49Peer::FECCOM)) $criteria->add(Tabla49Peer::FECCOM, $this->feccom);
		if ($this->isColumnModified(Tabla49Peer::DEBCRE)) $criteria->add(Tabla49Peer::DEBCRE, $this->debcre);
		if ($this->isColumnModified(Tabla49Peer::CODCTA)) $criteria->add(Tabla49Peer::CODCTA, $this->codcta);
		if ($this->isColumnModified(Tabla49Peer::NUMASI)) $criteria->add(Tabla49Peer::NUMASI, $this->numasi);
		if ($this->isColumnModified(Tabla49Peer::REFASI)) $criteria->add(Tabla49Peer::REFASI, $this->refasi);
		if ($this->isColumnModified(Tabla49Peer::DESASI)) $criteria->add(Tabla49Peer::DESASI, $this->desasi);
		if ($this->isColumnModified(Tabla49Peer::MONASI)) $criteria->add(Tabla49Peer::MONASI, $this->monasi);
		if ($this->isColumnModified(Tabla49Peer::ID)) $criteria->add(Tabla49Peer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(Tabla49Peer::ID, $this->id);
