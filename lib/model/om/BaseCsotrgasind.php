<?php


abstract class BaseCsotrgasind extends BaseObject  implements Persistent {


	
	protected static $peer;


	
	protected $codgas;


	
	protected $desgas;


	
	protected $cosgas;


	
	protected $tipmon;


	
	protected $id;

	
	protected $alreadyInSave = false;

	
	protected $alreadyInValidation = false;

  
  public function getCodgas()
  {

    return trim($this->codgas);

  }
  
  public function getDesgas()
  {

    return trim($this->desgas);

  }
  
  public function getCosgas($val=false)
  {

    if($val) return number_format($this->cosgas,2,',','.');
    else return $this->cosgas;

  }
  
  public function getTipmon()
  {

    return trim($this->tipmon);

  }
  
  public function getId()
  {

    return $this->id;

  }
	
	public function setCodgas($v)
	{

    if ($this->codgas !== $v) {
        $this->codgas = $v;
        $this->modifiedColumns[] = CsotrgasindPeer::CODGAS;
      }
  
	} 
	
	public function setDesgas($v)
	{

    if ($this->desgas !== $v) {
        $this->desgas = $v;
        $this->modifiedColumns[] = CsotrgasindPeer::DESGAS;
      }
  
	} 
	
	public function setCosgas($v)
	{

    if ($this->cosgas !== $v) {
        $this->cosgas = Herramientas::toFloat($v);
        $this->modifiedColumns[] = CsotrgasindPeer::COSGAS;
      }
  
	} 
	
	public function setTipmon($v)
	{

    if ($this->tipmon !== $v) {
        $this->tipmon = $v;
        $this->modifiedColumns[] = CsotrgasindPeer::TIPMON;
      }
  
	} 
	
	public function setId($v)
	{

    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = CsotrgasindPeer::ID;
      }
  
	} 
  
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {

      $this->codgas = $rs->getString($startcol + 0);

      $this->desgas = $rs->getString($startcol + 1);

      $this->cosgas = $rs->getFloat($startcol + 2);

      $this->tipmon = $rs->getString($startcol + 3);

      $this->id = $rs->getInt($startcol + 4);

      $this->resetModified();

      $this->setNew(false);

      $this->afterHydrate();

            return $startcol + 5; 
    } catch (Exception $e) {
      throw new PropelException("Error populating Csotrgasind object", $e);
    }
  }


  protected function afterHydrate()
  {

  }
    
  
  public function __call($m, $a)
    {
      $prefijo = substr($m,0,3);
    $metodo = strtolower(substr($m,3));
        if($prefijo=='get'){
      if(isset($this->$metodo)) return $this->$metodo;
      else return '';
    }elseif($prefijo=='set'){
      if(isset($this->$metodo)) $this->$metodo = $a[0];
    }else call_user_func_array($m, $a);

    }

	
	public function delete($con = null)
	{
		if ($this->isDeleted()) {
			throw new PropelException("This object has already been deleted.");
		}

		if ($con === null) {
			$con = Propel::getConnection(CsotrgasindPeer::DATABASE_NAME);
		}

		try {
			$con->begin();
			CsotrgasindPeer::doDelete($this, $con);
			$this->setDeleted(true);
			$con->commit();
		} catch (PropelException $e) {
			$con->rollback();
			throw $e;
		}
	}

	
	public function save($con = null)
	{
		if ($this->isDeleted()) {
			throw new PropelException("You cannot save an object that has been deleted.");
		}

		if ($con === null) {
			$con = Propel::getConnection(CsotrgasindPeer::DATABASE_NAME);
		}

		try {
			$con->begin();
			$affectedRows = $this->doSave($con);
			$con->commit();
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollback();
			throw $e;
		}
	}

	
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;


						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CsotrgasindPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setNew(false);
				} else {
					$affectedRows += CsotrgasindPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}

			$this->alreadyInSave = false;
		}
		return $affectedRows;
	} 
	
	protected $validationFailures = array();

	
	public function getValidationFailures()
	{
		return $this->validationFailures;
	}

	
	public function validate($columns = null)
	{
		$res = $this->doValidate($columns);
		if ($res === true) {
			$this->validationFailures = array();
			return true;
		} else {
			$this->validationFailures = $res;
			return false;
		}
	}

	
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			$this->alreadyInValidation = true;
			$retval = null;

			$failureMap = array();


			if (($retval = CsotrgasindPeer::doValidate($this, $columns)) !== true) {
				$failureMap = array_merge($failureMap, $retval);
			}



			$this->alreadyInValidation = false;
		}

		return (!empty($failureMap) ? $failureMap : true);
	}

	
	public function getByName($name, $type = BasePeer::TYPE_PHPNAME)
	{
		$pos = CsotrgasindPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
		return $this->getByPosition($pos);
	}

	
	public function getByPosition($pos)
	{
		switch($pos) {
			case 0:
				return $this->getCodgas();
				break;
			case 1:
				return $this->getDesgas();
				break;
			case 2:
				return $this->getCosgas();
				break;
			case 3:
				return $this->getTipmon();
				break;
			case 4:
				return $this->getId();
				break;
			default:
				return null;
				break;
		} 	}

	
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		$keys = CsotrgasindPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getCodgas(),
			$keys[1] => $this->getDesgas(),
			$keys[2] => $this->getCosgas(),
			$keys[3] => $this->getTipmon(),
			$keys[4] => $this->getId(),
		);
		return $result;
	}

	
	public function setByName($name, $value, $type = BasePeer::TYPE_PHPNAME)
	{
		$pos = CsotrgasindPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
		return $this->setByPosition($pos, $value);
	}

	
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				$this->setCodgas($value);
				break;
			case 1:
				$this->setDesgas($value);
				break;
			case 2:
				$this->setCosgas($value);
				break;
			case 3:
				$this->setTipmon($value);
				break;
			case 4:
				$this->setId($value);
				break;
		} 	}

	
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		$keys = CsotrgasindPeer::getFieldNames($keyType);

		if (array_key_exists($keys[0], $arr)) $this->setCodgas($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setDesgas($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setCosgas($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setTipmon($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setId($arr[$keys[4]]);
	}

	
	public function buildCriteria()
	{
		$criteria = new Criteria(CsotrgasindPeer::DATABASE_NAME);

		if ($this->isColumnModified(CsotrgasindPeer::CODGAS)) $criteria->add(CsotrgasindPeer::CODGAS, $this->codgas);
		if ($this->isColumnModified(CsotrgasindPeer::DESGAS)) $criteria->add(CsotrgasindPeer::DESGAS, $this->desgas);
		if ($this->isColumnModified(CsotrgasindPeer::COSGAS)) $criteria->add(CsotrgasindPeer::COSGAS, $this->cosgas);
		if ($this->isColumnModified(CsotrgasindPeer::TIPMON)) $criteria->add(CsotrgasindPeer::TIPMON, $this->tipmon);
		if ($this->isColumnModified(CsotrgasindPeer::ID)) $criteria->add(CsotrgasindPeer::ID, $this->id);

		return $criteria;
	}

	
	public function buildPkeyCriteria()
	{
		$criteria = new Criteria(CsotrgasindPeer::DATABASE_NAME);

		$criteria->add(CsotrgasindPeer::ID, $this->id);

		return $criteria;
	}

	
	public function getPrimaryKey()
	{
		return $this->getId();
	}

	
	public function setPrimaryKey($key)
	{
		$this->setId($key);
	}

	
	public function copyInto($copyObj, $deepCopy = false)
	{

		$copyObj->setCodgas($this->codgas);

		$copyObj->setDesgas($this->desgas);

		$copyObj->setCosgas($this->cosgas);

		$copyObj->setTipmon($this->tipmon);


		$copyObj->setNew(true);

		$copyObj->setId(NULL); 
	}

	
	public function copy($deepCopy = false)
	{
				$clazz = get_class($this);
		$copyObj = new $clazz();
		$this->copyInto($copyObj, $deepCopy);
		return $copyObj;
	}

	
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new CsotrgasindPeer();
		}
		return self::$peer;
	}

} 