...
abstract class BaseTabla15 extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->refaju !== $v) {
        $this->refaju = $v;
        $this->modifiedColumns[] = Tabla15Peer::REFAJU;
      }
    ...
    if ($this->codpre !== $v) {
        $this->codpre = $v;
        $this->modifiedColumns[] = Tabla15Peer::CODPRE;
      }
    ...
    if ($this->monaju !== $v) {
        ...
        $this->modifiedColumns[] = Tabla15Peer::MONAJU;
      }
    ...
    if ($this->stamov !== $v) {
        $this->stamov = $v;
        $this->modifiedColumns[] = Tabla15Peer::STAMOV;
      }
    ...
    if ($this->refprc !== $v) {
        $this->refprc = $v;
        $this->modifiedColumns[] = Tabla15Peer::REFPRC;
      }
    ...
    if ($this->refcom !== $v) {
        $this->refcom = $v;
        $this->modifiedColumns[] = Tabla15Peer::REFCOM;
      }
    ...
    if ($this->refcau !== $v) {
        $this->refcau = $v;
        $this->modifiedColumns[] = Tabla15Peer::REFCAU;
      }
    ...
    if ($this->refpag !== $v) {
        $this->refpag = $v;
        $this->modifiedColumns[] = Tabla15Peer::REFPAG;
      }
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = Tabla15Peer::ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      throw new PropelException("Error populating Tabla15 object", $e);
    }
  }
  ...
  public function __call($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = Tabla15Peer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += Tabla15Peer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Tabla15Peer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Tabla15Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = Tabla15Peer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Tabla15Peer::REFAJU)) $criteria->add(Tabla15Peer::REFAJU, $this->refaju);
		if ($this->isColumnModified(Tabla15Peer::CODPRE)) $criteria->add(Tabla15Peer::CODPRE, $this->codpre);
		if ($this->isColumnModified(Tabla15Peer::MONAJU)) $criteria->add(Tabla15Peer::MONAJU, $this->monaju);
		if ($this->isColumnModified(Tabla15Peer::STAMOV)) $criteria->add(Tabla15Peer::STAMOV, $this->stamov);
		if ($this->isColumnModified(Tabla15Peer::REFPRC)) $criteria->add(Tabla15Peer::REFPRC, $this->refprc);
		if ($this->isColumnModified(Tabla15Peer::REFCOM)) $criteria->add(Tabla15Peer::REFCOM, $this->refcom);
		if ($this->isColumnModified(Tabla15Peer::REFCAU)) $criteria->add(Tabla15Peer::REFCAU, $this->refcau);
		if ($this->isColumnModified(Tabla15Peer::REFPAG)) $criteria->add(Tabla15Peer::REFPAG, $this->refpag);
		if ($this->isColumnModified(Tabla15Peer::ID)) $criteria->add(Tabla15Peer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(Tabla15Peer::ID, $this->id);
