<?php
/**
 * Presupuesto Base: Clase estática para el manejo de las solicitudes de egreso
 *
 * @package    Roraima
 * @subpackage compras
 * @author     $Author: dmartinez $ <desarrollo@cidesa.com.ve>
 * @version SVN: $Id: SolicituddeEgresos.php 42325 2011-02-03 19:38:07Z dmartinez $
 *
 * @copyright  Copyright 2007, Cide S.A.
 * @license    http://opensource.org/licenses/gpl-2.0.php GPLv2
 */
class PresupuestoBase
{
  ...
  public static function grabarSolicitudEgreso($solegreso,$grid,$grid2,$grid3,$generar,&$msj)
  {
    ...
    $c = new Criteria();
    $obj = LidefempPeer::doSelectOne($c);
    if($obj)
        $r = $obj->getPrebas();
    else
        $r='';
    
      if ($solegreso->getReqart()=='########' && $r!='')
      {
	        $encontrado=false;
	        while (!$encontrado)
	        {
	          ...
	          $sql="select reqart from liprebas where reqart='".$numsol."' or reqart='".$numsol2."'";
     ...
     if ($msj==-1)
     {
      ...
      if($solegreso->getStareq()!='A' && $solegreso->getStareq()!='N')
      $solegreso->setStareq('P');
      $solegreso->save();
  ...
  public static function eliminarSolicitud($solegreso)
  {
    ...
    Herramientas::EliminarRegistro('Liprebasdet','Reqart',$codigo);    
    Herramientas::EliminarRegistro('Liprebasraz','Numsol',$codigo);
    $solegreso->delete();
  }
