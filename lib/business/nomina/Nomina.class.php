        ...
        if (Herramientas :: BuscarDatos($sql, & $tabla)) {
		  ...
		  }else
		  {
	  		if ($token = "STAB") {
	            ...
	            $sql = "Select ABS(SUM(case when a.pascar=coalesce('".$tabla[0]["paso"]."','001') then a.suecar else a.suecar*-1)) as suecar from NPCOMOCP A,NPCARGOS B WHERE B.CODCAR='" . $tabla[0]["codcar"] . "' AND A.CODTIPCAR=B.CODTIP AND A.GRACAR=B.GRAOCP AND (A.PASCAR='001' OR A.PASCAR='" . $tabla[0]["paso"] . "') AND FECDES<=TO_DATE('" . $movconvar . "','DD/MM/YYYY') ORDER BY FECDES DESC";
	          }
	          if (Herramientas :: BuscarDatos($sql, & $tablaescala)) {
	            $valor = $tablaescala[0]["suecar"];
	          } else {
